...
OUTPUT_PATH="$ROOT_DIR/docs/codex/SKILL_CANDIDATES.md"
TOP_N=10
...
Defaults:
  ...
  --output $ROOT_DIR/docs/codex/SKILL_CANDIDATES.md
  --top    10
USAGE
}
...
lines.append("")
lines.append("```bash")
lines.append("scripts/codex/draft-skill-from-candidate.sh <candidate-name>")
lines.append("```")
lines.append("")
...
if not entries:
    ...
    lines.append("")
    lines.append("```bash")
    lines.append("scripts/codex/skill-mine.sh")
    lines.append("```")
...
if not repeat_clusters:
    ...
    lines.append("Try rerunning after more sessions, or lower clustering thresholds in `scripts/codex/skill-mine.sh` if needed.")
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"[skill-mine] wrote {output_path}")
    sys.exit(0)
...
for idx, cand in enumerate(ranked, start=1):
    ...
    lines.append("")
    lines.append("```bash")
    lines.append(f"scripts/codex/draft-skill-from-candidate.sh {cand['skill_name']}")
    lines.append("```")
    lines.append("")
