...
CANDIDATES_FILE="$ROOT_DIR/docs/codex/SKILL_CANDIDATES.md"
GENERATOR_SCRIPT="$ROOT_DIR/skills/skill-generator/scripts/create_skill.sh"
...
Description:
  Extract a candidate WORKFLOW SPEC from docs/codex/SKILL_CANDIDATES.md and draft a skill
...
if [ ! -f "$CANDIDATES_FILE" ]; then
  ...
  echo "hint: run scripts/codex/skill-mine.sh first" >&2
  exit 1
fi
...
SKILL_DIR="$ROOT_DIR/skills/$SPEC_NAME"
if [ -d "$SKILL_DIR" ]; then
...
echo "Do not overwrite existing skills. Update docs/codex/USAGE.md with invocation guidance."
echo
cat "$TMP_SPEC"
echo "------------------------------------------------------------"
