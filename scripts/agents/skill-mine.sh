...
INPUT_PATH="$ROOT_DIR/dev/logs/agent-runs.ndjson"
OUTPUT_PATH="$ROOT_DIR/docs/agent-workflows/SKILL_CANDIDATES.md"
TOP_N=10
...
Defaults:
  --input  $ROOT_DIR/dev/logs/agent-runs.ndjson
  --output $ROOT_DIR/docs/agent-workflows/SKILL_CANDIDATES.md
  --top    10
USAGE
}
...
lines.append("")
lines.append("Manual creation required:")
lines.append("")
lines.append("```text")
lines.append("Use the WORKFLOW SPEC below to manually create a new skill in .opencode/skills/")
lines.append("```")
lines.append("")
...
if not repeat_clusters:
    ...
    lines.append("Try rerunning after more sessions, or lower clustering thresholds in `scripts/agents/skill-mine.sh` if needed.")
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"[skill-mine] wrote {output_path}")
    sys.exit(0)
...
for idx, cand in enumerate(ranked, start=1):
    ...
    lines.append("")
    lines.append("```text")
    lines.append(f"Use the WORKFLOW SPEC above to manually create the {cand['skill_name']} skill in .opencode/skills/")
    lines.append("```")
    lines.append("")
