...
CI_TEST_WORKFLOW="${WORKFLOWS_DIR}/ci-test.yml"
...
expect_pattern 'HELM_XCODE_ARCH: arm64' "$CI_TEST_WORKFLOW" "protected Xcode CI must pin arm64"
expect_pattern 'MACOSX_DEPLOYMENT_TARGET: "13\.0"' "$CI_TEST_WORKFLOW" "protected Xcode CI must pin the Ventura deployment target"
expect_pattern 'check_macos_ventura_compatibility\.sh' "$CI_TEST_WORKFLOW" "protected Xcode CI must validate its Ventura arm64 contract"
expect_pattern 'ARCHS=arm64' "$CI_TEST_WORKFLOW" "protected Xcode CI must build only arm64"
