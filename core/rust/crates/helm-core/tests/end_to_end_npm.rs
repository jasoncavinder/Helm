...
const VERSION_FIXTURE: &str = include_str!("fixtures/npm/version.txt");
const INSTALLED_FIXTURE: &str = include_str!("fixtures/npm/list_global.json");
const OUTDATED_FIXTURE: &str = include_str!("fixtures/npm/outdated_global.json");
const SEARCH_FIXTURE: &str = include_str!("fixtures/npm/search_array.json");
...
impl ProcessExecutor for NpmFakeExecutor {
        ...
        let (status, stdout): (ProcessExitStatus, Vec<u8>) = if program.ends_with("which") {
            ...
            match args.as_slice() {
                ...
                [arg0, arg1, arg2] if arg0 == "outdated" && arg1 == "-g" && arg2 == "--json" => {
                    ...
                    (ProcessExitStatus::ExitCode(0), stdout)
                }
...
#[tokio::test]
async fn npm_detect_list_search_and_mutate_through_orchestration() {
    let runtime = build_runtime(Arc::new(NpmFakeExecutor::new()));

    let detect_task = runtime
        .submit(ManagerId::Npm, AdapterRequest::Detect(DetectRequest))
        .await
        .unwrap();
