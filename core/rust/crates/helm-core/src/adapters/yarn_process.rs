...
impl YarnSource for ProcessYarnSource {
    ...
    fn list_installed_global(&self) -> AdapterResult<String> {
        let request = self.configure_request(yarn_list_installed_request(None));
    ...
    fn list_outdated_global(&self) -> AdapterResult<String> {
        ...
        let request = self.configure_request(yarn_list_outdated_request(None));
    ...
    fn search(&self, query: &str) -> AdapterResult<String> {
        ...
        // yarn may return exit code 1 for no matches while still writing JSON/JSONL output.
        self.run_and_collect_stdout_accepting(request, &[1])
    }
