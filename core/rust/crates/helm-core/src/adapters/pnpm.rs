    ...
    fn execute(&self, request: AdapterRequest) -> AdapterResult<AdapterResponse> {
        ...
        match request {
            ...
            AdapterRequest::Refresh(_) => {
                ...
                let installed = parse_pnpm_list_installed(&self.source.list_installed_global()?)?;
                let outdated = parse_pnpm_outdated(&self.source.list_outdated_global()?)?;
            ...
            AdapterRequest::Install(install_request) => {
                ...
                let after_version = install_request.version.clone().or_else(|| {
                    resolve_installed_pnpm_version(
                        &self.source,
                        install_request.package.name.as_str(),
                    )
                    .ok()
...
fn resolve_installed_pnpm_version<S: PnpmSource>(
    source: &S,
    package_name: &str,
) -> AdapterResult<Option<String>> {
    Ok(parse_pnpm_list_installed(&source.list_installed_global()?)?
        .into_iter()
        .find(|package| package.package.name == package_name)
        .and_then(|package| package.installed_version))
}

fn require_installed_pnpm_version<S: PnpmSource>(
    source: &S,
    package_name: &str,
) -> AdapterResult<String> {
    resolve_installed_pnpm_version(source, package_name)?.ok_or_else(|| CoreError {
        manager: Some(ManagerId::Pnpm),
        task: Some(TaskType::Uninstall),
        action: Some(ManagerAction::Uninstall),
        kind: CoreErrorKind::InvalidInput,
        message: format!("pnpm package '{package_name}' is not installed"),
    })
}

fn find_pnpm_outdated_entry<S: PnpmSource>(
    source: &S,
    package_name: &str,
) -> AdapterResult<Option<OutdatedPackage>> {
    Ok(parse_pnpm_outdated(&source.list_outdated_global()?)?
        .into_iter()
        .find(|package| package.package.name == package_name))
}
