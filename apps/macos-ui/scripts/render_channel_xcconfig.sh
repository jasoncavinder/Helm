...
CHANNEL_SPARKLE_ALLOW_DOWNGRADES=$(normalize_xcconfig_bool "$(read_xcconfig_value "$CHANNEL_TEMPLATE" HELM_SPARKLE_ALLOW_DOWNGRADES)")
...
if [ -n "${HELM_CHANNEL_OVERRIDE_SPARKLE_ALLOW_DOWNGRADES:-}" ]; then
    CHANNEL_SPARKLE_ALLOW_DOWNGRADES=$(normalize_xcconfig_bool "$HELM_CHANNEL_OVERRIDE_SPARKLE_ALLOW_DOWNGRADES")
fi
...
if [ "$CHANNEL_SPARKLE_ALLOW_DOWNGRADES" = "YES" ]; then
    echo "Invalid channel config: Sparkle downgrades must remain disabled." >&2
    exit 1
fi

cat > "$OUTPUT_PATH" <<XCCONFIG
// Auto-generated by render_channel_xcconfig.sh — do not edit
...
HELM_SPARKLE_ALLOW_DOWNGRADES = $CHANNEL_SPARKLE_ALLOW_DOWNGRADES
XCCONFIG
