...
Focus:
- continue the disposable-VM adapter campaign in `docs/validation/v0.20-adapter-vm-family-results.md`: uv endpoint lifecycles and the documented language-manager fixes pass their bounded scenarios; keep all campaign PRs unmerged until the owner reviews the completed batch. Finish durable cross-process task reservation, operation-specific CLT prerequisites, pnpm compatibility and the remaining live matrix. The original 29-manager absence/detection/policy baseline is not all-adapter certification; #500/#526 remain open, with signed GUI, supported-OS, cancellation, authenticated-source, account-dependent and guarded-system evidence still outstanding.
