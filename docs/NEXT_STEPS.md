...
Focus:
- preserve the recorded Task 7 exact-source owner evidence on `e87d30d3`: direct preview dismissal from Brief, Plan, and Receipt; same-artifact/same-database non-persistence; fixed-size Light/Dark flow; bidirectional Full Keyboard Access; VoiceOver stage focus, order, roles, and copy announcement; live German/Hungarian/Japanese localization; representative +40% expansion; strict aggregate summary; and selected missing-corpus unavailability. This is owner QA, not participant validation. Complete detailed VoiceOver state semantics for recommendation requirements, verified progress, receipt and unchanged-state facts, and action availability before proceeding to the owner-moderated participant checkpoint; keep whole-workflow cross-cutting accessibility, Ventura, and targeted Sparkle-handoff gates explicit
