...
- continue the native privileged-helper sequence from `docs/architecture/PRIVILEGED_EXECUTION.md`: preserve the now-embedded helper target, launch-daemon metadata, exact signed-caller checks, fixed Apple-operation allowlist, and release packaging verification; next add explicit `SMAppService` registration/approval UX and runtime activation, then resolve MAS and MacPorts without granting a root daemon broad access to unsigned or dynamic command surfaces
