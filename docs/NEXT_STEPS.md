...
- preserve the tracked-clean Environment Brief accessibility closure from 2026-08-26: merged `dev` source `86f4ef23db5081cfce6c595f7d088e1f323a1023`, tree `0597de3a7817347eb5ce3fd7b6dabc570c88141c`, 339 arm64 Xcode tests with 0 failures, executable SHA-256 `3dd0cf112f0c2703974d2d3dded7bf44e08c5b491e5cd6634f2b00e14696a83f`, and `Helm.debug.dylib` SHA-256 `7b210e581e9491475c04d723c4acd9b7411be13be26bfa0c92a2b992c496c6d8`. The same artifact passed ordinary wide and Debug-forced vertical VoiceOver Headings/focus geometry, separate `Software sources, 50%` announcement, symmetric Full Keyboard Access traversal, and unclipped layout. Issue #388 and the combined Task 7 accessibility item are closed. This is owner QA, not participant validation, and the Environment Brief stays development-gated until the production first-run route receives separate review and approval
- preserve PR #535 code source `410abbf9`: the separate noninteractive Course Indicator announces title before a zero-fraction-digit percentage formatted with Helm's selected locale, and the 339-test arm64 UI gate plus all-locale composed-label regression pass. Retain the `8964cef7`/`72b35019` owner artifact only as bounded pre-commit, plausibly source-equivalent historical evidence because its changed objects compiled before commit `4b465342`; the later tracked-clean `86f4ef23` matrix supplies the attributable closure evidence
- preserve PR #534 source `2c8726a0` and its stable responsive Environment Brief header, native title heading, separate noninteractive percentage, title-before-count readiness announcements, and 338-test arm64 gate. Treat the 2026-08-25 owner artifact as bounded unattributed pre-commit historical evidence because no exact source revision, tracked-clean tree, or source-blob equivalence mapping was retained; the later tracked-clean `86f4ef23` matrix supplies the required rotor, focus-geometry, bidirectional Full Keyboard Access, and wide/vertical layout evidence
- preserve the recorded Task 7 exact-source owner evidence on `e87d30d3`: direct preview dismissal from Brief, Plan, and Receipt; same-artifact/same-database non-persistence; fixed-size Light/Dark flow; scoped VoiceOver stage focus, order, and roles; copy-success announcement; live German/Hungarian/Japanese localization; representative +40% expansion; strict aggregate summary; selected missing-corpus unavailability; and detailed recommendation/progress/receipt/action-state semantics. Preserve the later `86f4ef23` closure of the combined Environment Brief accessibility item. These are owner-QA records rather than participant validation. Proceed toward the owner-moderated participant checkpoint while keeping whole-workflow cross-cutting accessibility, Ventura, targeted Sparkle-handoff, and the separate production first-run route gates explicit
...
- continue the v0.20 `uv` global-tool adapter from the staged read integration, guarded scope resolver, installation policy, and pure candidate eligibility in `docs/validation/uv-tool-eligibility.md`: next bind receipt/interpreter/distribution acquisition to the selected read scope and implement source-aware constrained resolution. PEP 440/Requires-Python filtering is not dependency resolution or execution authorization, and an incomplete latest-only listing cannot prove a constrained tool current. Preserve non-executing concrete mise/asdf enumeration, conservative install-layout attribution, and disabled uv executable lifecycle actions; complete tool-store permission/ownership checks before reviewed mutations. Layout evidence is not package-owner proof. Direct discovery, alias collapse, `uv tool dir`, and before/after filesystem identity checks protect the scoped read path; missing stores and ambiguous installations do not yield an authoritative inventory. Preserve shared task execution, bounded private capture, cancellation, and cache-preserving persistence; the provisional 0.12.9-0.12.18 version gate is not the final support policy. Keep local search, network-aware discovery, CLI/GUI runtime registration, post-action reconciliation, and the full real adapter lifecycle matrix explicit; this bounded integration does not close Issue #526 or certify the adapter under Issue #500
...
- preserve the unified Wayfinder popover implementation: ordinary states use one fixed `400x458` footprint with Helm identity/completed-check freshness, one shared-projection Course truth, a quiet System/Tools/Apps/Packages route, one condition-specific context fact, and Open Dashboard/Find Software/Check Again commands; each route stage opens Environment with a visible removable domain filter, Needs Review additionally selects the affected manager and names the known finding, and every contextual route carries its originating condition plus stable entity/focus/domain targets; Settings, Helm update checks, Support, About, and Quit live in a native utility menu; Library owns package search, Plan owns upgrade confirmation, Activity owns output/recovery, and Environment owns manager detail; offline/service state remains neutral and cannot submit refresh work
- preserve the completed Slice 20.1 state/accessibility matrix and canonical `v0.20-whole-workflow-v1` corpus recorded in `docs/validation/v0.20-wayfinder-popover-research-readiness.md`; preserve Task 2's exact read-only projection through the shipping Plan, inspector, selected-row risk model, and bounded confirmation plus its completed current-host visual/Full Keyboard Access/VoiceOver matrix. Slice 20.2 has a Ventura-compatible native authority-grouped outline, contextual trailing-toolbar manager/search filters, Wayfinder row and selection treatment, and automated empty/single/125-step, fixed-exclusion, stale-selection, stale-confirmation, partial-failure, cancellation, selected-risk, and hidden-selection coverage. Preserve Plan as the only bulk-update confirmation owner: Dashboard routes to Plan; notification Upgrade All clears stale Plan filters, selects current eligible rows, and requests the same reviewed workflow without enabling macOS updates; and the final sheet displays the exact execution order, automatic/interactive split, vendor handoffs, and selected privilege/reboot risk while failing closed on drift. Slice 20.3 now preserves immediate Cached Homebrew truth, bounded Remote Cargo enrichment, explicit offline deferral, deliberate accepted-result navigation, exact source-specific inspector context, the read-only install proposal, and the versioned production result-provenance contract that separates logical result origin from manager-search discovery while loss-isolating unknown or malformed nested data. Preserve the shared production/fixture Ventura-compatible native Library table, its exact consolidated-member inspector/action target, native selection and keyboard/type navigation, localized context/action commands, and restart/pin/status/version facts. Its merged-head current-host visual, Full Keyboard Access, and VoiceOver regression passed on 2026-08-19. Preserve generation-bound production remote-search cancellation: XPC/FFI submissions distinguish interactive and description request purposes and Rust in-flight deduplication includes that purpose; only task IDs owned by a superseded interactive query are cancelled, including late stale submissions; the dedicated local-runtime cancellation path keeps the 500 ms near-completion grace, treats verified already-terminal Search races as benign, and surfaces real failures; and snapshot reconciliation retires explicitly owned IDs on terminal status or after two consecutive authoritative snapshot absences without adopting unrelated work. Preserve the completed 20,000-row automated performance gate recorded in `docs/validation/v0.20-library-performance.md`: nonempty search uses the indexed identity-preserving overlay and shared production projection cache; cold indexed and end-to-end p95 are `135.458/195.415 ms`; warm indexed median/p95 are `22.365/24.332 ms` and warm production end-to-end p95 is `30.273 ms`; 117 of 120 AppKit scroll-critical samples stay within `16.7 ms`, worst is `43.616 ms`, and only 13 row views are retained. Preserve Task 4's strict read-only Activity projection and its provenance-scoped owner evidence: applied and verified are independent facts, the unstarted source remains explicitly unchanged, before/after and rollback limits stay visible, Restore remains unavailable without a verified downgrade path, recovery review cannot submit service work, the broad static matrix belongs to base `42c64760`, and the source-equivalent pre-commit snapshot later recorded as `c1f7daf` only adds bounded owner evidence for visible Inspector locale refresh without reselection and approximately +40% reachability. Preserve the independent-review automated evidence for structural inspector identity, representative `@State` and `@StateObject`, representative Boolean presentation state, and child lifecycle continuity across a locale change. Keep real sheet or popover presentation, scroll position, keyboard focus, and VoiceOver focus continuity open absent a remediated-head owner or production-integration retest. Preserve Task 1's shared fail-closed ambient projection, fixed research footprint, exact MacPorts/npm route identity, Activity `7001` inspector/focus handoff, and completed visual, interaction, Full Keyboard Access, VoiceOver, localization, invalid-corpus, and +40% expansion matrix on attributable PR head `3de57a81`; retain the earlier 13:02 HST build as directional evidence only. Preserve Task 5's fail-closed Environment projection, exact all-manager validation, explicit unavailable presentation for selected invalid/missing data, source-state-first health, exact active/manageable user rustup versus inactive/policy-blocked system rustup facts, read-only policy-versus-permission explanation, reversible fixture-local Keep Multiple acknowledgment, live inspector localization, selected-row `.isSelected` semantics, explicit Active/Inactive instance badges and accessibility values, disclosure-state accessibility, corrected Hungarian health labels, and their targeted regression. Treat the `d60aad9b` pass as historical superseded evidence only. Retain the `d2351667` owner matrix only for unaffected behavior, Full Keyboard Access, localization, and layout observations; independent review proved that its no-Hungarian-English-fallback and complete-VoiceOver-semantics claims were inaccurate. Preserve the attributable exact-source Task 5 closure matrix on `9e3fa24a`, including inactive-badge minimum/expanded Light/Dark/+40% layout, selected/inactive VoiceOver semantics, live Hungarian presentation and accessibility-title refresh, locale-specific Settings labels, and independent Settings behavior; preserve Task 6's fail-closed Settings/diagnostics projection, fixture-local launch-at-login changes, blocked `SMAppService` access, exact failed-Activity diagnostics route, and completed owner visual, interaction, accessibility, localization, layout, diagnostics, reversibility, and window-focus matrix on exact app source `43f15385`; and preserve Task 7's bounded exact-source owner evidence on `e87d30d3`, including detailed recommendation/progress/receipt/action-state semantics, while preserving the tracked-clean closure of Issue #388 and keeping the separate production-route decision open. Run owner-moderated sessions only after every protocol surface consumes the corpus, using the redacted templates under `docs/validation/templates/`, and keep the checkpoint explicitly open until participant thresholds are met
- preserve PR #528 Task 5 source through `890cf4fe`: the Hungarian in-progress Refreshing value is `Frissítés folyamatban`, locale changes reapply the hidden AppKit Dashboard and Settings accessibility titles, and the 321-test app-target gate covers both contracts. Treat the 13:56 HST owner artifact as bounded unattributed pre-commit evidence because it predates `061e9fa2` and has no retained source-tree/blob mapping. Preserve the completed current-source owner closure matrix from exact app source `9e3fa24a` and tracked-clean tree `fdfc45a8`, including the hashed binary, canonical corpus, and resource-only +40% expansion evidence recorded in the readiness contract. Track the separate Dashboard full-screen Space reopening defect under issue `#529`; retain the exact-source Task 6 canonical-flow owner matrix on `43f15385` and the completed remediated unavailable/safety-blocked Settings matrix on exact source `c80d362c`, then preserve Task 7's bounded exact-source evidence, including its detailed recommendation/progress/receipt/action-state semantics, without reopening Tasks 5 or 6 or the closed Issue #388 Environment Brief accessibility gate.
- preserve the independent-review Task 6 safety remediation: neither whole-workflow nor popover fixtures may construct/read/mutate the privileged-helper `SMAppService` path or inspect/install/remove the host CLI shim; missing/invalid Task 6 data and other active fixtures must present localized noninteractive unavailable truth instead of a fabricated Boolean or enabled no-op switch. Preserve the 328-test arm64 gate and focused zero-read/zero-mutation regressions.
- preserve locale-neutral missing package-version state: localized unknown labels belong only to presentation, global search must resolve the active locale without reselection or refetch, and semantic snapshots plus update-notification fingerprints must treat every supported legacy unknown token as the same absent version.
- preserve the Plan's vertical Wayfinder card-list presentation while retaining its native authority-grouped outline, virtualized scrolling, full-row inspector selection, leading inclusion controls, vendor actions, trailing status alignment, filter-stable disclosure choices, keyboard behavior, and bounded confirmation contract
...
- keep the Environment Brief development-gated in the published RC7 line pending a separately reviewed production first-run route decision; Issue #388 is resolved, but its owner-QA closure does not imply that the production route changed
- continue from the Original Wayfinder foundation checkpoints: shared revisioned Dashboard/popover/status projection, semantic Course Indicator modes, stable destination/deep-link bridge, Dashboard naming, restored/resizable native window hosting, native Dashboard/Plan/Library/Activity sidebar composition, persistent Environment access, rendered Course Indicator, native Dashboard toolbar/app commands, and one shared Settings window with General, Updates, Sources, CLI, and Support panes that follows the active Space beside full-screen apps and remains visible after Helm deactivates without depending on Dashboard. Dashboard and Settings navigation now use top-level semantic `NavigationSplitView` sidebar columns so Ventura and newer macOS releases own title-bar integration, resizing, collapse state, dividers, and standard toggles; preserve the title-free full-size chrome, Wayfinder brand/navigation/status content, pale-blue-to-near-white surface beneath the window controls, Dashboard menu-command synchronization, solid detail surfaces, and frame-stable in-detail inspector while completing the current-host plus Ventura-compatible validation matrix in `docs/validation/v0.20-native-sidebar-readiness.md`. One shared status-item popover for primary and secondary clicks, Dashboard-owned Service Health, and removal of the legacy in-window Settings destination are implemented; destination content migration, contextual inspector behavior, and v0.22 multi-display validation continue
- maintain release-process hardening guardrails for stable monitoring, any `0.19.x` maintenance, and the next prerelease line
...
- preserve the published `0.19.0-rc.4` manager/updater and native-experience baseline: v0.18 migration safety, npm/MAS reliability fixes, full standard-root Sparkle inventory and static-appcast visibility, external-bundle GUI update actions, scheduled direct-channel Helm checks, explicit beta/RC opt-in with stable-channel isolation, dependency modernization, the macOS 13 Ventura baseline, and the Original Wayfinder Dashboard/Settings foundation plus the System Default language migration correction
- preserve updater-plan truth: scheduled Helm checks discover availability without presenting Sparkle UI; available Helm updates appear consistently in Library, the sidebar badge, and Plan even when the service preview is empty or temporarily unavailable; the default-off Upgrade All preference runs Helm last through Sparkle's confirmation/relaunch flow; third-party Sparkle apps remain visible interactive steps rather than falsely counted automatic work, with consistent Open App to Update actions plus an in-app and background-notification handoff after automatic Plan work finishes; Mac App Store updates remain individually visible and every Plan step defaults to selected while exact step selection, filtered Select All/Deselect All behavior that preserves hidden selections, and selected-row risk flags are enforced; routine updates remain distinct from aggregate health, rely on the Plan badge instead of duplicating Attention in the Dashboard sidebar footer, and retain update semantics on the menu bar and popover; detected manager cards retain priority dragging while Not Installed cards cannot initiate reordering; and the default-on notification policy alerts on newly observed updates or required interaction rather than routine task completion, suppresses and clears update-availability alerts during backend Plan execution through the first accepted post-execution snapshot, consumes the reviewed Helm-only Plan fingerprint without conflating unrelated manual Sparkle checks, and routes notification actions through Plan and its existing confirmation/safety controls
- preserve the RC6 third-party Sparkle correction: sandboxed Helm does not offer an external installation path that downloads successfully but cannot complete the bundle-specific Sparkle installer handoff; Open App to Update is the truthful action across Plan, Library, popover, and inspectors. Deliver true direct updating in `0.20.x` through a dedicated signed/notarized external-updater boundary with structured request validation, installed-version verification, representative cross-app QA, and no weakening of Helm's app sandbox
...
- Original Wayfinder's Dashboard shell now uses Helm's bundled anchor app icon in its sidebar brand, has a native toolbar, and provides localized app commands for search, refresh, workspace routing, and sidebar/contextual-inspector visibility. All direct Settings routes converge on one compact SwiftUI `Settings` scene with General, Updates, Sources, CLI, and Support panes; pane selection is deferred outside SwiftUI's active update transaction, pane title keys are locale-validated, and the obsolete quick-Settings overlay, duplicate CLI card, manual Replay Walkthrough control, and colliding Support Helm badge are gone. Support Helm retains the intended full two-row action height. Refresh submission now fails closed when its XPC proxy is unavailable and popover reopening does not resubmit in-flight work. Service Health and its diagnostics snapshot action now live on Dashboard, combine manager detection/availability into one neutral coverage metric, and retain Dashboard deep links; the legacy in-window Settings destination and compatibility wrapper are removed, duplicate Refresh/Quit controls are absent from Settings, and manager-priority restoration lives with Sources policy. The development-gated Environment Brief anchors its completion actions in the left column, extends the source list to the same bottom edge, scrolls only that list, and fills the left-column gap with a truthful Management Readiness summary. Issue #388 is closed; the existing production onboarding route remains authoritative until the separate default-route review is complete.
- Published RC4 unifies primary and secondary status-item clicks on the live-localized popover, projects Helm and third-party Sparkle availability into Plan independently of the backend preview, and keeps automatic Helm participation in Upgrade All explicitly default-off and sequenced last.
- The RC3 language fix replaces the legacy English-backed System Default picker value with an explicit versioned preference. Fresh installs and the old ambiguous English sentinel resolve from macOS preferred languages, explicit legacy non-English selections are preserved, and all choices made through the new picker persist as explicit user intent.
- Owner installed-candidate evidence confirms that Sparkle advanced `/Applications/Helm.app` from RC2 to the signed Developer ID `0.19.0-rc.3` build and relaunched it with `user_locale_preference_v2=system` against the current `en-US` macOS application language. The app follows later Settings language changes; one explicit-language quit/relaunch persistence check remains before release-closeout documentation can call the full manual language matrix complete.
- Project WOW's first runtime foundation exposes a deterministic Environment Brief from existing manager-status and detection-task snapshots. It preserves a stable brief ID, increments revision only when semantic observations change, records current/cached/failed/cancelled coverage, remains `local_only`, and carries no command, consent, recommendation, or mutation state. Versioned first-use/current/partial/offline/service-failure fixtures are selectable only in debug builds through `HELM_ENVIRONMENT_BRIEF_FIXTURE`; persisted presentation state restores the legal/discovering/brief stage and revalidates manager selection. `HELM_ENVIRONMENT_BRIEF_FIRST_RUN=enabled` enables the new route only for unfinished onboarding, while `preview` presents it without resetting a completed development profile; `HELM_ENVIRONMENT_BRIEF_APPEARANCE=light|dark` provides deterministic appearance review. The status item then opens the real Dashboard window with legal, discovery, trust, system, coverage, and source states; fixture previews do not start service discovery. The 2026-08-08 presentation checkpoint passed the five-state minimum/expanded light/dark matrix and German/Hungarian/Japanese/+40% text expansion after correcting mapped-coverage progress and system-fact wrapping. Owner-assisted accessibility QA subsequently found the missing VoiceOver heading and asymmetric Full Keyboard Access traversal tracked in Issue #388; the attributable 2026-08-26 matrix closes that manual accessibility gate. The existing onboarding route remains authoritative until a separate default-switch review is complete.
...
- `v0.19.1` is the current public stable release; it carries forward the `v0.19.0` native-experience and resilience work, the `v0.18.2` doctor/repair and local security groundwork, migration recovery/safety, package workflow hardening, and the planning-only pre-1.0 experience-contract baseline:
  - bulk and scoped upgrade phase coordination now resides in Rust/FFI/XPC. The backend derives the current safe plan, schedules authority phases, waits for each phase to reach terminal state, and stops later-phase scheduling on scoped-workflow cancellation. Current in-process scoped selected-Plan FFI workflows additionally wait for each submitted task's ordered domain-response persistence turn to finish before advancing or publishing completion; the receipt does not claim that an underlying write succeeded, and legacy external-coordinator workflows retain terminal-only wait semantics. Individual tasks remain the live execution and diagnostics surface.
  ...
  - CLI delivery completed on `dev`:
    ...
    - CI runtime maintenance delivered: all official Node-based GitHub Actions currently used by the repository are immutable-pinned to Node.js 24-compatible releases, eliminating runner-forced Node 20 compatibility warnings; the CI toolchain contract now prevents legacy or floating refs from being reintroduced for both `actions/*` and `github/codeql-action`, while the upgraded `actions/*` set requires Actions Runner 2.327.1 or newer
