...
- continue the v0.20 `uv` global-tool adapter from the staged core read integration and scope resolver in `docs/validation/uv-global-tools-scope-discovery.md`: complete installation ownership/eligibility and concrete version-manager selection, then resolve Python-version/constraint/source eligibility before offering reviewed mutations. Direct executable discovery, symlink alias collapse, `uv tool dir` resolution, and before/after filesystem identity checks now protect the scoped read path; missing stores and ambiguous installations do not yield an authoritative inventory. Preserve shared task execution, bounded private capture, cancellation, and cache-preserving persistence; the provisional 0.12.9-0.12.18 version gate is not the final support policy. Keep local search, network-aware discovery, CLI/GUI runtime registration, post-action reconciliation, and the full real adapter lifecycle matrix explicit; this bounded integration does not close Issue #526 or certify the adapter under Issue #500
