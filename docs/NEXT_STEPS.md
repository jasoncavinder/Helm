...
Focus:
- close the Task 7 owner matrix on the final whole-workflow projection: launch the paired Debug-only corpus and first-run preview with an isolated `HELM_DB_PATH`; verify the partial Environment Brief, reviewed safe recommendation, synthetic verified progress, Action Receipt, aggregate-only copied summary, no onboarding persistence, and missing/invalid-data unavailability in Light/Dark, Full Keyboard Access, VoiceOver, supported representative locales, and +40% expansion. The implementation and focused arm64 contract/localization evidence are complete; earlier Task 7 “next” wording below is superseded
