...
```
v0.19.0-rc.1 published-candidate validation and v0.19.0-rc.2 preparation
```
...
- prepare `v0.19.0-rc.2` as the next prerelease after the native Settings and Wayfinder shell cohort is coherent, the full quality gate passes, and owner manual shell/window validation is recorded; keep the Environment Brief debug-gated in `rc.2` if Issue #388 remains unresolved and disclose that limitation explicitly
- continue from the Original Wayfinder foundation checkpoints: shared revisioned Dashboard/popover/status projection, semantic Course Indicator modes, stable destination/deep-link bridge, Dashboard naming, restored/resizable native window hosting, native Dashboard/Plan/Library/Activity sidebar composition, persistent Environment access, rendered Course Indicator, native Dashboard toolbar/app commands, one shared native Settings scene, and a schema-aligned local-only Environment Brief projection with deterministic debug fixtures, resumable presentation state, and a debug-gated Dashboard-hosted first-run view are implemented; the five-state minimum/expanded light/dark matrix and German/Hungarian/Japanese/+40% text-expansion presentation checks are complete; owner-assisted QA found the VoiceOver Headings rotor and bidirectional Full Keyboard Access defect tracked in Issue #388, so the existing onboarding remains authoritative while Settings pane migration, destination content migration, contextual inspector behavior, right-click-menu removal, and remaining manual window validation continue
