...
- continue the v0.20 `uv` global-tool adapter from the staged core read integration in `docs/validation/uv-global-tools-read-integration.md`: add selected executable/tool-store discovery and ownership, then resolve Python-version/constraint/source eligibility before offering reviewed mutations. Explicit-scope offline inventory now uses shared task execution, bounded private capture, cancellation, and cache-preserving persistence; the provisional 0.12.9-0.12.18 version gate is not the final support policy. Preserve the opt-in isolated command/parser evidence and empty-store process smoke, but keep local search, network-aware discovery, CLI/GUI runtime registration, post-action reconciliation, and the full real adapter lifecycle matrix explicit; this bounded integration does not close Issue #526 or certify the adapter under Issue #500
