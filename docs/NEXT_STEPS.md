...
Focus:
- preserve PR #535 code source `410abbf9`: the separate noninteractive Course Indicator now announces title before a zero-fraction-digit percentage formatted with Helm's selected locale, and the 339-test arm64 UI gate plus all-locale composed-label regression pass. Retain the `8964cef7`/`72b35019` owner artifact only as bounded pre-commit, plausibly source-equivalent evidence because its changed objects compiled before commit `4b465342`; keep Issue #388 and the combined Task 7 accessibility item open until the wide/forced-vertical matrix is rerun from a recorded tracked-clean current-source checkout
- preserve PR #534 source `2c8726a0` and its stable responsive Environment Brief header, native title heading, separate noninteractive percentage, title-before-count readiness announcements, and 338-test arm64 gate. Treat the 2026-08-25 owner artifact as bounded unattributed pre-commit evidence because no exact source revision, tracked-clean tree, or source-blob equivalence mapping was retained. Keep Issue #388 and the combined Task 7 accessibility item open until the owner reruns the rotor, focus-geometry, bidirectional Full Keyboard Access, and wide/vertical layout matrix against an attributable exact-source artifact
- preserve the recorded Task 7 exact-source owner evidence on `e87d30d3`: direct preview dismissal from Brief, Plan, and Receipt; same-artifact/same-database non-persistence; fixed-size Light/Dark flow; scoped VoiceOver stage focus, order, and roles; copy-success announcement; live German/Hungarian/Japanese localization; representative +40% expansion; strict aggregate summary; selected missing-corpus unavailability; and detailed recommendation/progress/receipt/action-state semantics. This is owner QA, not participant validation. Remediate and retest the Environment Brief heading-rotor and bidirectional Full Keyboard Access defect in Issue #388 before proceeding to the owner-moderated participant checkpoint, while keeping whole-workflow cross-cutting accessibility, Ventura, and targeted Sparkle-handoff gates explicit
