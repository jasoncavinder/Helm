...
- post-publication release verification is green for the `v0.19.0-rc.7` GUI beta appcast, RC CLI metadata, GitHub prerelease state, and stable/RC coexistence contract
- validation gates are green through published RC7: candidate `cargo test`, 153 arm64 macOS `xcodebuild` tests, locale integrity/length audits, release contracts, clean-`main` rehearsal/preflight/canary, signed/notarized GUI and direct CLI publication, and all three post-publication verification paths passed
