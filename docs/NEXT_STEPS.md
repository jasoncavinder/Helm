...
Focus:
- preserve PR #534 source `2c8726a0` and its stable responsive Environment Brief header, native title heading, separate noninteractive percentage, title-before-count readiness announcements, and 338-test PR gate. Retain the earlier 2026-08-25 pre-commit artifact as bounded unattributed evidence. Preserve the attributable exact-source closure matrix on `4b465342` and tree `7e8a425b`: both wide and Debug-forced-vertical layouts passed the Headings menu, matching focus geometry, `Software sources, 50%` announcement order, symmetric bidirectional Full Keyboard Access, Management Readiness order/case, and unclipped layout checks; the exact-source full arm64 UI gate passes 339 tests with 0 failures. This closes Issue #388's acceptance criteria but does not approve the production first-run route or constitute participant validation
- preserve the recorded Task 7 exact-source owner evidence on `e87d30d3`: direct preview dismissal from Brief, Plan, and Receipt; same-artifact/same-database non-persistence; fixed-size Light/Dark flow; scoped VoiceOver stage focus, order, and roles; copy-success announcement; live German/Hungarian/Japanese localization; representative +40% expansion; strict aggregate summary; selected missing-corpus unavailability; and detailed recommendation/progress/receipt/action-state semantics. Combined with the exact-source `4b465342` Environment Brief accessibility matrix, the complete-flow owner item is closed. This is owner QA, not participant validation; whole-workflow cross-cutting accessibility, Ventura, targeted Sparkle handoff, moderated research, and the separately reviewed production first-run route remain explicit gates
...
- keep the Environment Brief development-gated in the published RC7 line until the production first-run route is separately reviewed and approved; Issue #388's accessibility acceptance criteria are closed, but no released first-run route changed
