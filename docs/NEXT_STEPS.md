...
```
v0.19.0-rc.2 post-publication remediation and v0.19.0 stabilization
```

Focus:
- merge and validate the post-RC2 language-default hotfix, then carry it in a new immutable prerelease candidate before stable `v0.19.0`: fresh and migrated installs must begin in a true System Default mode that follows macOS, while subsequent explicit language choices persist
...
- The post-RC2 language hotfix replaces the legacy English-backed System Default picker value with an explicit versioned preference. Fresh installs and ambiguous legacy state resolve from macOS preferred languages, a one-time migration removes the old value, and all choices made through the new picker persist as explicit user intent.
