...
Focus:
- validate the v0.20 Dashboard Add manager flow from issue #463 using `docs/validation/v0.20-add-manager-dashboard.md`: initial owner card/chooser/navigation/cancellation checks passed; recheck scroll-to-manager handoff, Cargo/Homebrew choices for cargo-binstall, and details-toggle placement after search, then finish filtered-route, offline, appearance, keyboard/VoiceOver/localization checks. Keep actual installation/failure/cancellation/recovery evidence in an isolated environment and the all-manager certification pass (#500); do not install managers on the owner's Mac as an automated test
