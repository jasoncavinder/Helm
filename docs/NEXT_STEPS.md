...
Focus:
- finish the targeted installation-review language recheck for the v0.20 Dashboard Add manager flow from issue #463 using `docs/validation/v0.20-add-manager-dashboard.md`: source `3876f7c0` passed owner navigation/cancellation, scroll/filter handoff, Cargo/Homebrew choices, toolbar, appearance, keyboard/VoiceOver, and offline-safety checks; untranslated policy/setup/timeout labels were the remaining reported presentation defect. All six non-English catalogs now translate those labels and adjacent alternate-source controls; recheck the signed replacement without reopening accepted layout work. Synchronize the PR with `dev`, complete independent review and final-head CI, and keep actual installation/failure/cancellation/recovery evidence in an isolated environment and the all-manager certification pass (#500); do not install managers on the owner's Mac as an automated test
