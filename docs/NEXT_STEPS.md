...
```
v0.19.0-rc.4 source-candidate validation and v0.19.0 stabilization
```

Focus:
- validate the prepared `v0.19.0-rc.4` source candidate without tagging or publishing until protected `dev` and `main` integration, final clean-`main` gates, and explicit release authorization are complete
- preserve published stable `v0.18.2` and prerelease `v0.19.0-rc.3` metadata while RC4 remains untagged
- after publication, validate the installed `v0.19.0-rc.3` to signed/notarized `v0.19.0-rc.4` Sparkle update path and the integrated Helm update-plan behavior
...
- keep the Environment Brief development-gated in RC4 because Issue #388 remains unresolved, and disclose the limitation without implying that the production first-run route changed
...
- maintain release-process hardening guardrails while RC4 moves through source-candidate validation and protected integration
...
- preserve updater-plan truth: scheduled Helm checks discover availability without presenting Sparkle UI; available Helm updates appear consistently in Library, the sidebar badge, and Plan even when the service preview is empty or temporarily unavailable; the default-off Upgrade All preference runs Helm last through Sparkle's confirmation/relaunch flow; third-party Sparkle apps remain visible interactive steps rather than falsely counted automatic work, with explicit vendor-update/Open App actions plus an in-app and background-notification handoff after automatic Plan work finishes; and the default-on notification policy alerts on newly observed updates or required interaction rather than routine task completion, with notification actions routed through Plan and its existing confirmation/safety controls
...
- The RC4 source candidate unifies primary and secondary status-item clicks on the live-localized popover, projects Helm and third-party Sparkle availability into Plan independently of the backend preview, and keeps automatic Helm participation in Upgrade All explicitly default-off and sequenced last.
...
- prepared source candidate: `v0.19.0-rc.4`; it is not tagged or published, and public prerelease metadata remains on `v0.19.0-rc.3`
...
- post-publication release verification is green for the `v0.19.0-rc.3` GUI beta appcast, RC CLI metadata, GitHub prerelease state, and stable/RC coexistence contract; RC4 remains an untagged source candidate
