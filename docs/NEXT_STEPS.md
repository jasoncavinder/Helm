...
- run the focused RC5 matrix in `docs/validation/v0.19.0-rc.5-readiness.md` for the full post-RC4 stabilization delta, including PRs `#419` through `#428`
- continue the native privileged-helper sequence from `docs/architecture/PRIVILEGED_EXECUTION.md`: preserve the now-embedded `developer_id` helper target, launch-daemon metadata, non-direct-channel stripping, exact signed-caller checks, fixed Apple-operation allowlist, and release packaging verification; next add explicit `SMAppService` registration/approval UX and runtime activation, then resolve MAS and MacPorts without granting a root daemon broad access to unsigned or dynamic command surfaces
...
- do not treat the embedded privileged-helper foundation as active native authorization; the existing `sudo -A` askpass path remains the runtime implementation until registration, approval-state handling, executor activation, and signed installed-build QA pass together
