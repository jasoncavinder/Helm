...
- continue the v0.20 `uv` global-tool adapter from the staged read integration, guarded scope resolver, and installation policy in `docs/validation/uv-installation-policy.md`: preserve non-executing concrete mise/asdf candidate enumeration, conservative install-layout attribution, and disabled uv executable lifecycle actions; next resolve Python-version/constraint/source eligibility and safe candidates, then tool-store permission/ownership checks before reviewed mutations. Layout evidence is not package-owner proof. Direct executable discovery, alias collapse, `uv tool dir` resolution, and before/after filesystem identity checks protect the scoped read path; missing stores and ambiguous installations do not yield an authoritative inventory. Preserve shared task execution, bounded private capture, cancellation, and cache-preserving persistence; the provisional 0.12.9-0.12.18 version gate is not the final support policy. Keep local search, network-aware discovery, CLI/GUI runtime registration, post-action reconciliation, and the full real adapter lifecycle matrix explicit; this bounded integration does not close Issue #526 or certify the adapter under Issue #500
