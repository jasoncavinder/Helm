...
- preserve updater-plan truth: scheduled Helm checks discover availability without presenting Sparkle UI; available Helm updates appear consistently in Library, the sidebar badge, and Plan even when the service preview is empty or temporarily unavailable; the default-off Upgrade All preference runs Helm last through Sparkle's confirmation/relaunch flow; and third-party Sparkle apps remain visible interactive steps rather than falsely counted automatic work
