...
- `v0.18.2` is the current public stable release; doctor/repair, local security groundwork, migration recovery/safety, package workflow hardening, and planning-only pre-1.0 experience contracts are published:
  ...
  - CLI delivery completed on `dev`:
    ...
    - CI runtime maintenance delivered: all official Node-based GitHub Actions are immutable-pinned to Node.js 24 releases, eliminating runner-forced Node 20 compatibility warnings; the CI toolchain contract now prevents legacy or floating refs from being reintroduced
