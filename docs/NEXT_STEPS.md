...
- preserve the completed Slice 20.1 state/accessibility matrix and canonical `v0.20-whole-workflow-v1` corpus recorded in `docs/validation/v0.20-wayfinder-popover-research-readiness.md`; project that corpus through the production Plan, Library, Activity/recovery, Environment, Settings/diagnostics, and Project WOW surfaces as their v0.20 slices land, then run owner-moderated sessions using the redacted templates under `docs/validation/templates/`. Keep the checkpoint explicitly open until participant thresholds are met
