...
- run the focused RC5 matrix in `docs/validation/v0.19.0-rc.5-readiness.md` for the post-RC4 changes delivered by PRs `#419` through `#422`
- complete the native privileged-helper sequence from `docs/architecture/PRIVILEGED_EXECUTION.md`: preserve the typed fail-closed operation contract, add the signed/notarized `SMAppService` launch daemon and approval UX, enforce helper-side caller/request validation, and resolve MAS without granting a root daemon broad access to a user-owned unsigned executable
...
- do not treat the process-local privileged-executor foundation as shipped native authorization; the existing `sudo -A` askpass path remains the runtime fallback until helper packaging and installed-build QA pass together
