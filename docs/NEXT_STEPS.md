...
- continue from the Original Wayfinder foundation checkpoints: shared revisioned Dashboard/popover/status projection, semantic Course Indicator modes, stable destination/deep-link bridge, Dashboard naming, restored/resizable native window hosting, native Dashboard/Plan/Library/Activity sidebar composition, persistent Environment access, rendered accessible Course Indicator, native Dashboard toolbar/app commands, and one shared native Settings scene are implemented; migrate Settings into dedicated preference panes while relocating operational cards, then continue destination content migration, contextual inspector behavior, state fixtures, right-click-menu removal, and manual window/accessibility validation
