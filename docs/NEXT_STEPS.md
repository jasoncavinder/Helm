...
- continue the v0.20 `uv` global-tool adapter from the unregistered read-only foundation in `docs/validation/uv-global-tools-foundation.md`: bind supported executable/store provenance and task execution, integrate authoritative inventory without clearing cached data on parse failure, then resolve Python-version/constraint eligibility before offering reviewed mutations. Preserve the opt-in isolated uv 0.12.9/0.12.18 command/parser evidence, but keep CLI/GUI registration, post-action reconciliation, and the full real adapter lifecycle matrix explicit; neither parser fixtures nor the upstream-command harness close Issue #526 or certify the adapter under Issue #500
