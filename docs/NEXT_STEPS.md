...
- execute the Slice 20.1 state/accessibility matrix in `docs/validation/v0.20-wayfinder-popover-research-readiness.md` using the debug-only production-view fixture seam; then create a versioned whole-workflow synthetic dataset before owner-moderated sessions. Use the redacted per-session and aggregate templates under `docs/validation/templates/`, and keep the checkpoint explicitly open until participant thresholds are met
