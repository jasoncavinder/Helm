...
- continue the v0.20 `uv` global-tool adapter from the unregistered read-only foundation in `docs/validation/uv-global-tools-foundation.md`: bind supported executable/store provenance and task execution, integrate authoritative inventory without clearing cached data on parse failure, then resolve Python-version/constraint eligibility before offering reviewed mutations. Keep CLI/GUI registration, post-action reconciliation, and the isolated real lifecycle matrix explicit; parser fixtures do not close Issue #526 or certify the adapter under Issue #500
