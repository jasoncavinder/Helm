...
- preserve the narrowly scoped retained-Homebrew-keg correction merged for the upcoming `0.19.1` maintenance line: same-prefix historical kegs do not create false multi-install attention, distinct Homebrew prefixes remain visible, and inactive install entries cannot initiate manager-wide update or uninstall operations
...
- `v0.19.0` is the current public stable release; it carries forward the `v0.18.2` doctor/repair, local security groundwork, migration recovery/safety, package workflow hardening, and planning-only pre-1.0 experience-contract baseline alongside the v0.19 native-experience and resilience work:
