...
The v0.20 `uv` global-tool adapter is now registered in GUI/FFI and CLI. It provides guarded concrete-install discovery, local installed-tool search, source-aware constrained outdated resolution, reviewed per-tool install/upgrade/uninstall, and observed-result verification. Environment, Library, Plan, and Activity share the existing runtime/task/persistence path. Executable ownership remains separate: Helm does not self-update or remove uv through this adapter. The runtime currently accepts stable uv 0.12.9-0.12.18 and fails closed on unsupported receipt/source policy, unsafe storage, stale targets, and incomplete evidence. Implementation, limitations, and the pending all-adapter VM certification gate are recorded in `docs/validation/uv-global-tools-lifecycle.md`.

The earlier opt-in command/parser test passes on uv 0.12.9 and 0.12.18 with Python 3.14.7 on arm64 macOS. The new adapter lifecycle test uses a clean child environment and generated local wheels to verify constrained candidates, Python/dependency incompatibility, exact pins, reviewed upgrade, install, failed install, removal, and unrelated-tool preservation. Independent review added PEP 440-equivalent version verification: an observed `1.1` satisfies a requested `1.1.0`, and persistence retains the observed spelling; genuinely different or invalid versions still fail. These disposable offline stores do not touch host tools or the stable database. They are bounded automated evidence, not all-adapter GUI/CLI or Ventura runtime certification.
...
Uv runtime discovery integrates bounded, non-executing enumeration of concrete mise/asdf versions, direct-alias collapse, and explicit selection for distinct installations; retained Homebrew kegs remain excluded. The shared install-instance classifier continues to keep uv executable lifecycle actions read-only, including unknown-provenance overrides. Per-tool mutations additionally enforce user-owned storage and source/scope verification. Earlier layout evidence remains documented in `docs/validation/uv-installation-policy.md`.

The pure `adapters::uv_tool_eligibility` policy and exact-pinned `uv-pep440` 0.0.85 regressions remain available as filtering contracts, not execution authorization. The integrated adapter uses uv's full resolver with the supported receipt's requirements/extras/constraints, existing interpreter, and preserved source configuration. It does not treat `uv tool list --outdated` as a complete eligible-candidate set. Private bounded captures and source-build/Python-download restrictions apply. Unsupported advanced sources and receipts remain manual-review limitations; see `docs/validation/uv-global-tools-lifecycle.md`. Issues #526/#500 remain open pending certification.
