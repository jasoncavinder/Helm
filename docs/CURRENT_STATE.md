...
- current privileged-execution foundation requires every elevated process request to carry one stable `PrivilegedOperation` identity and rejects mismatched/untyped elevation state before spawn. A trusted absolute executor can be configured once inside the service process to receive that identity plus the resolved structured command; inherited environment cannot select it, and absent that seam the existing `sudo -A` askpass implementation remains active. Mac App Store mutations remain explicitly pinned to the existing askpass path even when a trusted executor is configured, because the Homebrew `mas` binary is user-owned and unsigned on the reference system. The native Service Management launch daemon, registration/approval UX, helper-side caller/operation validation, release embedding, signed installed-build QA, and bounded MAS replacement path are not yet implemented; `docs/architecture/PRIVILEGED_EXECUTION.md` records the completion gates.
