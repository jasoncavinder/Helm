...
- current privileged-execution foundation requires every elevated process request to carry one stable `PrivilegedOperation` identity and rejects mismatched/untyped elevation state before spawn. A trusted absolute executor can be configured once inside the service process to receive that identity plus the resolved structured command; inherited environment cannot select it, and absent that seam the existing `sudo -A` askpass implementation remains active. `developer_id` app bundles now embed a dual-mode `HelmPrivilegedHelper` executable plus its Service Management launch-daemon plist, and release automation explicitly signs and verifies the nested helper. Non-direct channels strip those artifacts so Developer ID-only helper assumptions do not leak into App Store, Setapp, or fleet bundles. The helper accepts only its exact signed client whose immediate parent is the exact signed Helm service, revalidates every request without a shell, limits execution to three fixed Apple `softwareupdate` shapes, bounds captured output, and terminates associated work when the caller disconnects. It is intentionally not registered or configured as the runtime executor yet, so user-visible authorization behavior is unchanged. Mac App Store and all MacPorts mutations remain pinned to askpass pending bounded designs for their unsigned/dynamic command surfaces. Registration/approval UX, runtime activation, signed installed-build approval/denial/cancellation QA, and a bounded MAS replacement path remain incomplete; `docs/architecture/PRIVILEGED_EXECUTION.md` records the completion gates.
