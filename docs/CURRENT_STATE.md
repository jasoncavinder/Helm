...
- 0.17.x — Diagnostics & Logging (**stable released on `main`**, RC lineage `v0.17.0-rc.1` through `v0.17.0-rc.5`)
  ...
  - delivered: `0.18.x` doctor/repair persistence and execution follow-up (on `dev`): migration 17 makes SQLite canonical for scoped scan generations, finding lifecycle, versioned repair knowledge, provenance/audit state, local overrides, and repair history; bundled knowledge is imported at store startup through the same strict RFC 8785/SHA-256 envelope contract used for external imports; planning at CLI and FFI/XPC surfaces resolves effective local knowledge against a compiled typed-action registry; hostile imports, trust escalation, protected rebinding, policy weakening, equivocation, and stale scan completion fail closed; repair apply revalidates active findings and the FFI task path records follow-up verification outcomes.
...
- Stage 1 (`0.18.x`): local security groundwork implemented on `dev` (normalized advisory domain records, deterministic cache identity/order, freshness contracts, task hooks, and migration 18 SQLite cache persistence)
