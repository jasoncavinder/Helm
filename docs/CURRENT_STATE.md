...
- prepared source candidate: **0.19.0-rc.5**; it is not tagged or published, and public beta/RC metadata remains on `v0.19.0-rc.4`. The release pipeline has no private signed/notarized candidate mode, so RC5 publication is the first valid installed-build registration test vehicle; native executor activation remains deferred until that post-publication evidence passes.
- RC5 pre-publication validation is complete: protected integration checks, clean-`main` Sparkle/rehearsal/preflight gates, release canary `31687166639`, publication-auth write probe `31687166770`, and owner-assisted ordinary-UI QA passed from the isolated `Helm-Development` namespace. Signed installation, Sparkle replacement, helper registration/approval, and persistence checks remain post-publication evidence and are not inferred from the development build.
...
- public default-channel GUI appcast and stable CLI metadata remain on signed `v0.18.2` artifacts, while the `beta` appcast item and `latest-rc.json` point to `v0.19.0-rc.4`.
