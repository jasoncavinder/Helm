...
Implementation baseline: **published `0.19.0-rc.1` validation on the v0.18.2 lineage**, combining manager/updater reliability, dependency modernization, and the macOS 13 baseline while preserving released migration safety. Original Wayfinder implementation is active on `dev` with its shared presentation projection, Dashboard window foundation, native workspace composition, rendered Course Indicator, and the read-only Project WOW Environment Brief runtime projection with deterministic debug fixtures and resumable presentation state; the broader destination redesign and first-run view replacement remain staged work beyond these foundation checkpoints.
