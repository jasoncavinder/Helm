...
- active Original Wayfinder implementation checkpoint on `dev`: one deterministic revisioned projection selects approval, failed/interrupted, active, actionable-environment, updates-ready, refreshing/unavailable, or healthy truth for Dashboard, popover, and status-item routing; determinate Course Indicator progress accepts only validated backend-owned completed/total values; stable Dashboard/Plan/Library/Activity/Environment deep links bridge to legacy sections during migration; the existing content host runs in a restored, resizable Dashboard window with `860x600` minimum sizing plus standard Command-1 and Command-Comma routes; the production shell now presents Dashboard, Plan, Library, and Activity as native sidebar peers, keeps Environment persistent at the sidebar foot, keeps Settings separate, and renders an accessible reduced-motion-aware Course Indicator from that shared projection; destination content still uses the legacy hosts during migration, and standard menu/toolbar completion, contextual inspector migration, full state fixtures, and right-click-menu removal remain subsequent slices
