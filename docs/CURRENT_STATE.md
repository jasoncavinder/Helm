...
The v0.20 `uv` global-tool work has a standalone Rust read-only command/output foundation in `adapters::uv_tool`, with synthetic regression fixtures. It preserves tool identities, executable children, installed requirements, and latest-version observations separately, and rejects failed, malformed, or diagnostic-bearing listings rather than returning partial inventory. It is not registered as a manager: no detection, refresh, planning, mutation, GUI/CLI exposure, or real tool lifecycle certification is enabled by this slice. Latest observations are not eligible upgrade targets. The contract and remaining activation gates are recorded in `docs/validation/uv-global-tools-foundation.md`.
