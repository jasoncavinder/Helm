...
- current updater-plan integration keeps scheduled direct-channel Helm checks information-only and projects discovered Helm updates into Helm's package and plan surfaces. A persisted, default-off setting may include Helm in Upgrade All; when enabled, all service/core work reaches terminal state before Sparkle presents its confirmation and relaunch flow. Third-party Sparkle updates are visible in the same reviewed plan as interactive steps but are never counted or dispatched as automatic bulk work. Fleet remains PKG/MDM-controlled; this slice establishes reusable policy and sequencing only.
