...
Implementation baseline: **published `0.19.0-rc.1` validation on the v0.18.2 lineage**, combining manager/updater reliability, dependency modernization, and the macOS 13 baseline while preserving released migration safety. Original Wayfinder implementation is active on `dev` with its shared presentation projection, Dashboard window foundation, native workspace composition, rendered Course Indicator, and the read-only Project WOW Environment Brief runtime projection with deterministic debug fixtures, resumable presentation state, and a debug-gated Dashboard-hosted first-run view. Its five-state minimum/expanded appearance and localized/text-expansion presentation matrix has passed after bounded remediation. Owner-assisted checks found the state title missing from the VoiceOver Headings rotor and asymmetric Full Keyboard Access traversal within the noninteractive header; Issue #388 tracks the unresolved gate, and the production first-run route remains on the existing onboarding flow pending its remediation and a separate default-route review.
