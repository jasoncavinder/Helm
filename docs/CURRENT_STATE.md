...
Implementation baseline: **published `0.19.0-rc.3` on the `0.19.0-rc.2` lineage**, retaining the Original Wayfinder native Dashboard/Settings foundation and limiting the release to the post-RC2 language-default correction. Owner validation confirmed both signed RC1-to-RC2 and RC2-to-RC3 Sparkle update/relaunch paths. The first exposed a language-default regression because the legacy preference conflated System Default with English; the second installed the corrected signed Developer ID build, migrated that sentinel to the explicit versioned System Default preference, and followed the current macOS application language. RC3 also preserves explicit legacy non-English selections and persists subsequent manual choices under regression coverage; one installed-app quit/relaunch check for a newly selected explicit language remains open. The read-only Project WOW Environment Brief runtime projection, deterministic debug fixtures, and resumable presentation state remain development-gated. Issue #388 tracks its unresolved VoiceOver Headings-rotor and bidirectional Full Keyboard Access behavior, so the production first-run route remains on the existing onboarding flow. Operational-card relocation and removal of the legacy in-window Settings compatibility destination remain outside this narrow stabilization release.
