...
- 0.17.x — Diagnostics & Logging (**stable released on `main`**, RC lineage `v0.17.0-rc.1` through `v0.17.0-rc.5`)
  ...
  - in progress: post-`v0.17.5` release-process hardening phase 3 policy alignment prep (on `dev`): preflight now validates `main` ruleset publish-PR bypass policy (GitHub Actions integration actor in `pull_request` mode, `Policy Gate` required-check presence, and no `always` bypass actors) and release operations docs now include exact verification expectations; live ruleset mutation is pending maintainer token/repository-permission scope that can update repository rulesets.
