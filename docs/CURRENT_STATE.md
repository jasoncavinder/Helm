...
Implementation baseline: **published `0.19.0-rc.2` opt-in candidate on the `0.19.0-rc.1` lineage**, combining the Original Wayfinder native Dashboard/window/sidebar/toolbar foundation with dedicated native Settings panes while preserving the manager/updater and migration-safety baseline. Owner validation confirmed the signed Sparkle update and relaunch path, then exposed a language-default regression: the legacy preference conflated System Default with English and could retain an unintended projected locale across the update. The post-RC2 hotfix introduces an explicit versioned System Default preference that follows macOS, resets the ambiguous legacy value once, and preserves subsequent manual choices. The read-only Project WOW Environment Brief runtime projection, deterministic debug fixtures, and resumable presentation state remain development-gated. Issue #388 tracks its unresolved VoiceOver Headings-rotor and bidirectional Full Keyboard Access behavior, so the production first-run route remains on the existing onboarding flow. Operational-card relocation and removal of the legacy in-window Settings compatibility destination are explicitly deferred until after this candidate.
