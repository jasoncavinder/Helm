...
- current Task 1 ambient-health projection consumes the canonical whole-workflow dataset through the shipping fixed-size Wayfinder popover. It identifies the exact applied-but-unverified `prettier` result, reports eight of ten sources available with one failed source needing review and current/cached/review/error route semantics, carries fixed checked freshness, and routes Review Recovery to exact Activity selection `7001`. The projector rejects identity, safety, scenario, coverage, failed-source-record, and activity drift; the selected fixture still blocks service startup, refresh, detection, persisted snapshots, updater traffic, and mutation. Automated arm64 evidence and the current-host owner visual/accessibility/localization matrix are complete.
