...
- Task 7 status: the final canonical workflow projection now runs inside the shipping first-run host only behind the paired Debug-only whole-workflow and Environment Brief preview gates. It presents the exact partial brief, reviewed no-network/no-privilege `mise` shell recommendation, verified synthetic progress, Action Receipt, and strict aggregate-only summary. Stage transitions remain in view memory; canonical or record-order drift and missing/invalid paired-preview data fail closed; and leaving the research flow never persists onboarding completion or starts service, database, network, updater, authorization, detection, or mutation work. Focused arm64 projection, transition, paired-gate completion-policy, redaction, drift, all-locale parity, manager-placeholder, responsive-header, and readiness-announcement-order regressions pass. The bounded canonical-flow, copied-summary, and same-database relaunch checks pass on app source `bfbea8c6`; the recorded exact-source owner checks on `e87d30d3` cover direct dismissal from every stage, Light/Dark, scoped VoiceOver stage focus/order/roles, copy-success announcement, live localization, representative +40% expansion, selected missing-corpus unavailability, and detailed recommendation/progress/receipt/action-state semantics. Source later committed unchanged as `2c8726a0` closes the remaining Environment Brief heading-rotor, focus-geometry, separate-percentage, and bidirectional Full Keyboard Access defect tracked in Issue #388 across ordinary wide and Debug-forced vertical layouts. This is owner QA, not participant validation or a whole-workflow cross-cutting accessibility claim.
