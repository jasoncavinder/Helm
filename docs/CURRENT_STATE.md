...
Implementation baseline: **published `0.19.0-rc.1` validation on the v0.18.2 lineage**, combining manager/updater reliability, dependency modernization, and the macOS 13 baseline while preserving released migration safety. Original Wayfinder implementation has now begun on `dev` with its shared presentation-projection and Dashboard window foundation; the broader native-experience and Project WOW implementation remains staged work beyond this first checkpoint.
...
- active Original Wayfinder implementation checkpoint on `dev`: one deterministic revisioned projection now selects approval, failed/interrupted, active, actionable-environment, updates-ready, refreshing/unavailable, or healthy truth for Dashboard, popover, and status-item routing; determinate Course Indicator progress accepts only validated backend-owned completed/total values; stable Dashboard/Plan/Library/Activity/Environment deep links bridge to legacy sections during migration; the existing content host now runs in a restored, resizable Dashboard window with `860x600` minimum sizing plus standard Command-1 and Command-Comma routes; the visual Course Indicator, native workspace sidebar, and right-click-menu removal remain subsequent slices
