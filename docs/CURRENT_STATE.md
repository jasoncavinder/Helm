...
- Task 7 status: the final canonical workflow projection now runs inside the shipping first-run host only behind the paired Debug-only whole-workflow and Environment Brief preview gates. It presents the exact partial brief, reviewed no-network/no-privilege `mise` shell recommendation, verified synthetic progress, Action Receipt, and strict aggregate-only summary. Stage transitions remain in view memory; canonical or record-order drift and missing/invalid paired-preview data fail closed; and leaving the research flow never persists onboarding completion or starts service, database, network, updater, authorization, detection, or mutation work. Focused arm64 projection, transition, paired-gate completion-policy, redaction, drift, all-locale parity, and manager-placeholder regressions pass. The bounded canonical-flow, copied-summary, and same-database relaunch checks pass on app source `bfbea8c6`; the recorded exact-source owner checks pass on `e87d30d3`, including direct dismissal from every stage, Light/Dark, bidirectional Full Keyboard Access, VoiceOver stage focus/order/roles and copy-success announcement, live localization, representative +40% expansion, and selected missing-corpus unavailability, including detailed VoiceOver state semantics. This is owner QA, not participant validation.
