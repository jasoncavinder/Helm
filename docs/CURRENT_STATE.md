...
Implementation baseline: **published `0.19.0-rc.3` on the `0.19.0-rc.2` lineage**, retaining the Original Wayfinder native Dashboard/Settings foundation and limiting the release to the post-RC2 language-default correction. Owner validation confirmed the signed RC1-to-RC2 Sparkle update and relaunch path, then exposed a language-default regression: the legacy preference conflated System Default with English. RC3 introduces an explicit versioned System Default preference that follows macOS, maps the ambiguous legacy English sentinel to System Default, preserves explicit legacy non-English selections, and persists subsequent manual choices. The read-only Project WOW Environment Brief runtime projection, deterministic debug fixtures, and resumable presentation state remain development-gated. Issue #388 tracks its unresolved VoiceOver Headings-rotor and bidirectional Full Keyboard Access behavior, so the production first-run route remains on the existing onboarding flow. Operational-card relocation and removal of the legacy in-window Settings compatibility destination remain outside this narrow stabilization release.
...
- owner installation validation confirms that Sparkle detected, installed, and relaunched the signed `v0.19.0-rc.2` update from `v0.19.0-rc.1`; the relaunch exposed an unintended German locale caused by the ambiguous legacy language preference, and `v0.19.0-rc.3` publishes the explicit macOS-following System Default fix while preserving explicit legacy non-English choices and every later explicit selection
...
- owner installation validation confirms `/Applications/Helm.app` reached the signed Developer ID `0.19.0-rc.2` build through Sparkle; RC2-to-RC3 installed-update validation remains pending, so this is still one successful owner installation/update-path validation, not broad participant validation.
