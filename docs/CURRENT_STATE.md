...
- current native-shell polish uses Helm's bundled anchor app icon in the Dashboard sidebar brand, keeps the selected Plan badge legible, combines detected and additionally available managers into one neutral Service Health coverage metric, gives the Settings scene a compact default and bounded layout, and presents Support Helm as a clean full-height action beside its two-button peer stack. Routine update availability no longer degrades aggregate Service Health; the sidebar footer instead presents a distinct localized Updates Available state that opens Plan, while running, error, and attention states route to their authoritative Activity, Environment, or Service Health context. The development-gated Environment Brief keeps its header, system summary, and completion actions anchored while only its software-source list scrolls; the source column extends beside the left-column actions so both columns share one bottom edge, and a tested Management Readiness summary classifies sources into mutually exclusive ready, attention, and observed-only counts. The obsolete manual Replay Walkthrough control is removed, while the existing production first-run onboarding route remains unchanged until the Environment Brief gate tracked by Issue #388 is resolved and separately approved.
