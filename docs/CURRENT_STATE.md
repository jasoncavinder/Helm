...
- 0.17.x — Diagnostics & Logging (**stable released on `main`**, RC lineage `v0.17.0-rc.1` through `v0.17.0-rc.5`)
  ...
  - delivered: post-`v0.17.7` doctor/repair foundation scaffold follow-up (on `dev`): added `helm-core` doctor and repair modules (deterministic finding fingerprinting, local health-report model, and a transitional embedded repair map), routed manager package-state mismatch detection through doctor findings (fingerprint/severity/evidence + repair options in manager-status payloads), added repair execution API through FFI/XPC (`helm_apply_manager_package_state_issue_repair`), migrated GUI metadata-only manager mismatch actions to the repair subsystem path, added local doctor visibility for recent known Homebrew lock-conflict failures without deleting Homebrew lockfiles, and added CLI doctor scaffolding (`helm doctor scan`, `helm doctor repair plan|apply`) for parity-ready health/repair workflows. The `0.18.x` target replaces the transitional map with SQLite-backed portable knowledge and a non-executable typed-action contract.
