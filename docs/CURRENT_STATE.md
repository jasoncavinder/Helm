...
The v0.20 `uv` global-tool work now includes a core-only read adapter in `adapters::uv_tool_process`, building on the strict command/output foundation in `adapters::uv_tool`. A caller supplies one immutable absolute executable/tool-store scope; the adapter rechecks the executable version before offline inventory and runs through shared task identity, timeout, cancellation, and persistence boundaries. Opt-in bounded private process capture keeps raw requirements and diagnostics out of task logs and rejects incomplete captures. Failed reads preserve committed inventory, and only confirmed empty inventory clears it. The registry has a staged `uv` identity with Detect/Refresh/ListInstalled capabilities, but the normal GUI/CLI runtime does not register it and FFI still reports it unimplemented. Automatic discovery/ownership, search, update candidates, mutations, and full lifecycle certification remain open. Latest observations are not eligible upgrade targets. The foundation and current evidence boundaries are recorded in `docs/validation/uv-global-tools-foundation.md` and `docs/validation/uv-global-tools-read-integration.md`.
