...
- superseding Task 7 status: the final canonical workflow projection now runs inside the shipping first-run host behind the paired Debug-only whole-workflow and Environment Brief preview gates. It presents the exact partial brief, reviewed no-network/no-privilege `mise` shell recommendation, verified synthetic progress, Action Receipt, and strict aggregate-only summary. Stage transitions remain in view memory; canonical or record-order drift and missing/invalid data fail closed; and leaving the research flow never persists onboarding completion or starts service, database, network, updater, authorization, detection, or mutation work. Focused arm64 projection, transition, completion-policy, redaction, drift, all-locale parity, and manager-placeholder regressions pass. The bounded owner canonical-flow, copied-summary, and same-database relaunch checks pass on app source `bfbea8c6`; exact-source Light/Dark, Full Keyboard Access, VoiceOver, localization, expansion, and unavailable-state QA remain open.
