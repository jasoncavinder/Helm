...
- 0.17.x — Diagnostics & Logging (**stable released on `main`**, RC lineage `v0.17.0-rc.1` through `v0.17.0-rc.5`)
  ...
  - delivered: post-`rc.3` operator ergonomics/perf follow-up: launch-at-login setting now supports macOS 11+ (`SMAppService.mainApp` on macOS 13+, embedded login-helper fallback via `SMLoginItemSetEnabled` on macOS 11/12), popover hover cursor forcing relaxed to restore clickable hover affordance, and manager/popover count rendering now uses precomputed dictionaries to reduce repeated per-row filtering during scroll/drag interactions.
  ...
  - delivered: post-`0.17.3` bundled-CLI shim follow-up (for `0.17.4`): app builds now package the `helm-cli` binary into `Helm.app` resources, Settings now exposes install/remove actions for a managed `~/.local/bin/helm` shim, shim runtime resolves the app-bundled CLI path and offers self-removal when the app bundle is missing, and shim installation writes install provenance as `channel=app-bundle-shim` with `update_policy=channel`.
  - delivered: post-`0.17.3` TUI planning follow-up (for `0.17.4`): ratatui-based TUI implementation plan is now documented at `docs/architecture/HELM_TUI_IMPLEMENTATION_PLAN.md`, covering keyboard navigation model, GUI parity targets, branding constraints, phased delivery, and required ASCII splash-screen content (`logo` + `Helm` + `Take the helm.`).
  - delivered: post-`0.17.3` TUI implementation follow-up (for `0.17.4`): no-arg TTY invocation now launches a ratatui/crossterm interactive TUI with branded ASCII splash, keyboard-first navigation, command palette/help/confirmation overlays, section panes for updates/packages/tasks/managers/settings/diagnostics, and direct mutation hooks for refresh/detect, selected package actions, task cancel, manager lifecycle toggles, and global upgrade workflow submission.
  - delivered: post-`0.17.3` TUI parity-expansion follow-up (for `0.17.4`): managers pane now supports keyboard-driven selected-manager detection, executable-path cycling, install-method cycling, and priority reordering; updates pane now supports toggleable `include_pinned` + `allow_os_updates` workflow options before `Upgrade All`; diagnostics pane now supports one-key JSON export snapshot writing; settings pane now embeds provenance-aware self-update status with check/apply (including guarded force path) using the same channel-policy constraints as `helm self ...`.
  ...
  - delivered: release-process hardening phase 6 docs alignment (on `docs`): release documentation now requires syncing release-prep branches with latest `origin/main` before final prep PRs and includes an explicit conflict-risk checklist (changelog, generated version files, release workflows, update metadata paths).
...
- CLI `self` namespace is now provenance-aware across supported channels (`direct-script`, `brew`, `macports`, `cargo`, `app-bundle-shim`, `managed`, `unknown`): `self status`/`self check`/`self update`/`self uninstall` enforce channel policy, provide channel-specific recommended actions, perform direct binary self-update only when policy allows, and support smart uninstall for direct-script/app-bundle-shim installs.
...
- Non-TUI GUI↔CLI parity-closure follow-up delivered on `dev`:
  - CLI search now supports progressive local+remote orchestration (`--local|--remote`, optional manager scope) with remote fan-out across enabled/detected/search-capable managers.
  - CLI now exposes per-package Homebrew keg-policy controls (`packages keg-policy list|get|set|reset`) aligned with GUI package-level overrides.
  - CLI updates preview/run now support manager-scoped bulk-upgrade execution (`--manager <id>`).
  - TUI packages pane now surfaces progressive available-package rows from cached/remote search enrichment and supports package install actions (`i`) alongside existing upgrade/uninstall/pin flows.
  - TUI packages pane now supports Homebrew per-package keg-policy override cycling (`K`) with explicit confirmation.
  - launch-at-login remains intentionally GUI-scoped; CLI/TUI do not expose this setting.
