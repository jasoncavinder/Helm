...
Implementation baseline: **published `0.19.0-rc.6`**, which retains the RC5 Dashboard/status-item, update-notification, Plan-selection, reconnection, manager-compatibility, and privileged-operation foundations while correcting Settings presentation in external full-screen Spaces, replacing the broken sandboxed third-party Sparkle installer handoff with a truthful vendor-owned `Open App to Update` flow, and moving pinned GitHub Actions to Node 24-compatible revisions. Public prerelease metadata now points to RC6 while stable GitHub/latest, appcast, and CLI metadata remains on `v0.18.2`. Installed-candidate replacement, Settings, preference-persistence, and third-party vendor-update checks remain explicit owner evidence rather than inferred publication results. The read-only Project WOW Environment Brief remains development-gated. Issue #388 still tracks its unresolved VoiceOver Headings-rotor and bidirectional Full Keyboard Access behavior, so the production first-run route remains on the existing onboarding flow.
