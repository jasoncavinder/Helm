...
- current privileged-execution foundation requires every elevated process request to carry one stable `PrivilegedOperation` identity and rejects mismatched/untyped elevation state before spawn. A trusted absolute executor can be configured once inside the service process to receive that identity plus the resolved structured command; inherited environment cannot select it, and absent that seam the existing `sudo -A` askpass implementation remains active. The native Service Management launch daemon, registration/approval UX, helper-side caller/operation validation, release embedding, and signed installed-build QA are not yet implemented. The Homebrew `mas` binary is user-owned and unsigned on the reference system, so it cannot be broadly executed by a persistent root helper; `docs/architecture/PRIVILEGED_EXECUTION.md` records the bounded options and completion gates.
