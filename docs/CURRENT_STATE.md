...
- current Slice 20.3 production Library results carry an additive version 1 provenance object from Rust through FFI/XPC JSON into Swift. The contract distinguishes a result's current delivery origin from its discovery source, so an item discovered by remote manager search and later read from SQLite is labeled Cached rather than Remote while retaining its remote-search query. Installed and outdated manager snapshots are Local. Swift validates schema version, source-manager identity, and allowed field combinations before showing the localized origin; older payloads remain decodable, and unknown or inconsistent provenance suppresses only the label. No SQLite migration is required. The native Library table, production remote-search cancellation, and 20,000-row performance gate remain open.
