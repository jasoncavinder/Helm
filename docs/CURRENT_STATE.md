...
- 0.17.x — Diagnostics & Logging (**stable released on `main`**, RC lineage `v0.17.0-rc.1` through `v0.17.0-rc.5`)
  ...
  - delivered: post-`v0.17.5` release-process hardening phase 6 (on `dev`): release docs now require release-branch sync against latest `origin/main` before final prep PRs and include a conflict-risk checklist (`CHANGELOG`, generated version artifacts, release workflows, update metadata paths) to reduce late-stage merge friction.
