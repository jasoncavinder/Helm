...
- The Sparkle apps manager inventories Sparkle bundles under `/Applications` and `~/Applications`, derives conservative outdated state only from static HTTPS appcasts using bounded concurrent checks and an aggregate scan budget, and delegates each selected update to Sparkle's external-bundle updater configured for that app. Helm is explicitly excluded and dynamic-feed apps remain inventory-only. Outdated third-party Sparkle apps appear in reviewed plans as interactive vendor-managed steps with explicit Update and Open App controls, but remain excluded from automatic bulk execution and manager lifecycle automation because Sparkle owns each app's confirmation and Helm has no reliable completion contract.
