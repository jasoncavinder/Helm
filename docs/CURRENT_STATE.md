...
Current documentation baseline: **0.19.0 is the latest public stable release on `main`** and **0.19.0-rc.7 remains the published opt-in prerelease**; `v0.18.0` remains withdrawn because of a critical SQLite migration defect.
...
Active milestone:
- latest stable release currently published on `main`: **0.19.0**, represented by a published non-prerelease GitHub release with signed/notarized GUI artifacts and direct CLI artifacts; GitHub `releases/latest` resolves to `v0.19.0`
...
- public default-channel GUI appcast and stable CLI metadata point to signed `v0.19.0` artifacts, while the `beta` appcast item and `latest-rc.json` remain on `v0.19.0-rc.7`.
...
0.13.x through 0.19.0 stable checkpoints are complete on `main`; `v0.18.0` remains withdrawn, `v0.18.1` is its corrective successor, `v0.18.2` is the final v0.18 containment release, and `v0.19.0` is the current stable release.
