...
Environment Brief accessibility closure: on 2026-08-26, one isolated arm64 Debug artifact built from tracked-clean merged `dev` source `86f4ef23db5081cfce6c595f7d088e1f323a1023` and tree `0597de3a7817347eb5ce3fd7b6dabc570c88141c` passed the 339-test Xcode suite with 0 failures and the complete owner-assisted ordinary-wide/Debug-forced-vertical VoiceOver and Full Keyboard Access matrix. The executable SHA-256 was `3dd0cf112f0c2703974d2d3dded7bf44e08c5b491e5cd6634f2b00e14696a83f`; the `Helm.debug.dylib` SHA-256 was `7b210e581e9491475c04d723c4acd9b7411be13be26bfa0c92a2b992c496c6d8`. The title appeared in the Headings menu with correct focus geometry, the separate ring announced `Software sources, 50%`, bidirectional traversal was symmetric, and both layouts were unclipped. This closes Issue `#388` and the combined Task 7 accessibility item, but it is owner QA rather than participant validation and does not approve the production first-run route switch. Any older checkpoint below saying this gate remains open is retained as chronological evidence and is superseded by this closure record.

The released `0.19.1` maintenance correction normalizes retained older Homebrew kegs under the same formula and Homebrew prefix into the active or linked logical manager installation. Separate Homebrew prefixes remain distinct, inactive installation paths no longer inherit the active executable's version, and manager-wide update/uninstall controls are limited to the active installation entry. This correction does not add automatic keg cleanup or change Homebrew ownership.

The v0.20 `uv` global-tool work includes a core-only read adapter in `adapters::uv_tool_process`, building on the strict command/output foundation in `adapters::uv_tool`. A caller supplies one immutable absolute executable/tool-store scope; the adapter rechecks the executable version before offline inventory and runs through shared task identity, timeout, cancellation, and persistence boundaries. The staged `adapters::uv_tool_scope` resolver can now search direct executable locations or honor an explicit selection, collapse symlink aliases, require selection for distinct copies, and query `uv tool dir` rather than guess storage. A resolved existing scope binds canonical paths and filesystem identities, rechecked before and after reads; missing storage is a non-authoritative observation, not empty inventory. Known shims require concrete-executable selection. This is read-scope identity, not installation ownership or mutation authorization. Opt-in bounded private process capture keeps raw requirements and diagnostics out of task logs and rejects incomplete captures. Failed reads preserve committed inventory, and only confirmed empty inventory clears it. The registry has a staged `uv` identity with Detect/Refresh/ListInstalled capabilities, but the normal GUI/CLI runtime does not register it and FFI still reports it unimplemented. Ownership/eligibility, search, update candidates, mutations, and full lifecycle certification remain open. Latest observations are not eligible upgrade targets. Evidence and remaining gates are recorded in `docs/validation/uv-global-tools-foundation.md`, `docs/validation/uv-global-tools-read-integration.md`, and `docs/validation/uv-global-tools-scope-discovery.md`.

A separate opt-in real-command test passes on uv 0.12.9 and 0.12.18 with Python 3.14.7 on arm64 macOS, using generated local wheels and isolated offline stores. It verifies parser output through constrained upgrades, exact pins, multiple executables, failed installation, broken-receipt rejection, restoration, and scoped uninstall. It neither changes host tools nor exercises a registered Helm adapter; the normal test suite leaves this harness ignored. Full adapter certification remains open.

On 2026-09-21, the owner confirmed the live-data Sparkle Plan/confirmation checkpoint in the source-verified, signed arm64 Debug app from `edb4270b`: Arc and iTerm2 appeared as interactive vendor handoffs, and the final review distinguished 101 selected rows from 99 automatic updates before cancellation. This is bounded presentation evidence, not a vendor installation or participant session. Exact artifact provenance and the excluded stable-app setup incident are recorded in `docs/validation/v0.20-wayfinder-popover-research-readiness.md`. The follow-up sequence-label layout reserves the largest number's digit width instead of wrapping at 100. The readiness branch also pins the protected Xcode check to arm64 with a macOS 13.0 deployment target; its green PR result remains required, and testing on a macOS 26 runner is not a Ventura runtime pass.

See:
- CHANGELOG.md

The staged uv core includes bounded, non-executing enumeration of concrete mise/asdf uv versions and conservative Homebrew/mise/asdf install-layout attribution. Directory aliases collapse, distinct concrete versions require explicit selection, and retained Homebrew kegs are not enumerated. The shared uv install-instance classifier remains read-only for all executable lifecycle actions, including unknown-provenance overrides; a layout hint is not package-owner proof or permission to mutate the global tool store. The existing read-scope guards remain authoritative. Details and evidence are in `docs/validation/uv-installation-policy.md`.

The pure `adapters::uv_tool_eligibility` policy now checks caller-supplied modern receipts against installed identity/entrypoints and evaluates observed registry candidates with PEP 440 constraints, Requires-Python, and yank/prerelease safeguards. Exact-pinned `uv-pep440` 0.0.85 replaces the initial matcher after independent review reproduced incorrect prerelease/epoch bounds and zero-padded wildcard membership; four regressions cover those cases, including Requires-Python. A matching candidate still needs full resolution; this is not network discovery, proof of a compatible dependency graph, or an automatic update target. Non-registry/private-source configuration, conditional or unfamiliar receipt policy, and missing evidence remain explicit limitations. Receipt/interpreter/metadata acquisition and scope binding, source-aware resolution, permission checks, mutations, and registration remain open. `docs/validation/uv-tool-eligibility.md` records the contract and next gates. This does not activate uv in the GUI/CLI or complete #526/#500.

The v0.20 Dashboard now ends Manager Health with a distinct Add manager card. Its chooser uses only implemented, undetected managers with core-published, planner-supported installation methods; missing metadata never falls back to the UI registry. Policy, eligibility, dependency readiness, active operations, network/service availability, and research-fixture safety gate review. Selection scrolls to the manager and opens the existing Environment inspector installation review rather than submitting work. That review revalidates the selected method before submission and after preference callbacks, preserves structured service execution and existing Activity recovery, and cannot bypass review when method metadata is empty. Cargo-binstall now supports an explicit Cargo installation method through the existing adapter (`cargo install cargo-binstall --locked`) alongside Homebrew; no searchable-package catalog is required. Refresh/Review Plan stay leading while native search and the separate details toggle stay trailing. Owner navigation, toolbar, appearance, keyboard/VoiceOver, and offline-safety checks passed on source `3876f7c0`; the remaining installation-panel language defect was corrected across all six non-English catalogs and accepted on signed source `0548920c`. The bounded current-host owner presentation matrix and independent code review are complete. Synchronization with `dev` adds only the merged website security lockfile update, preserving the reviewed app source; final-head CI remains required. Isolated installation-lifecycle evidence remains open under issues #463/#500. This does not certify live installation or change the public release.
...
- `v0.19.1` publication completed from tagged `main` revision `0319641b29b26c89d7c8a7e224e2091306e95fd2`; release canary `32136717070` and publication-auth write probe `32138158953` passed, release workflows `32138347077` and `32138347136` succeeded, generated CLI and appcast/release-notes PRs `#504` and `#505` merged, and converged metadata revision `ec9def28c3ef965574e6fb9afe93c0ec499901cc` passed workflow-specific verification, release verification, and both drift guards
- `v0.19.0` publication completed from tagged `main` revision `e3f148fdbaf87c78e2502c6a17ff5e13ae2d4147`; release canary `31937189914` and publication-auth write probe `31937193834` passed, release workflows `31937869680` and `31937869677` succeeded, generated CLI and appcast/release-notes PRs `#482` and `#483` merged, and converged metadata revision `1e8c4f55fb7f30995231fa329315b4579622e1a6` passed release verification plus both drift guards
- `v0.19.0-rc.7` publication completed from tagged `main` revision `a29ecb3924e0f029553ad9209c6bc431852a1ccd`; release canary `31922485485` and publication-auth write probe `31922485489` passed, release workflows `31923270915` and `31923270917` succeeded, generated CLI and appcast/release-notes PRs `#475` and `#476` merged, and converged metadata revision `feca8ac709d2969115ce5ef1e8026214511ac902` passed release verification plus both drift guards
- `v0.19.0-rc.6` publication completed from tagged `main` revision `968d0e400acc1a02a7e0a9a3d1a55917325189e8`; release canary `31861074375` and publication-auth write probe `31861074433` passed, release workflows `31861811608` and `31861811594` succeeded, generated CLI and appcast/release-notes PRs `#458` and `#459` merged, and converged metadata revision `9214c3ea430ea7d3cf8a422d2f7399e5df026dd5` passed release verification plus both drift guards
- `v0.19.0-rc.5` publication completed on 2026-08-13 from tagged `main` revision `7dfdc26f9dbc7d3ae75f105cef7f1d7c147a2da3`; release workflows `31690897042` and `31690897034` succeeded, generated CLI and appcast/release-notes PRs `#441` and `#442` merged, and final metadata revision `f5bb91ad643655dd67afae40c80ae2ecc16a2294` passed release verification plus both drift guards
- RC5 pre-publication validation completed through protected integration checks, clean-`main` Sparkle/rehearsal/preflight gates, release canary `31687166639`, publication-auth write probe `31687166770`, and owner-assisted ordinary-UI QA from the isolated `Helm-Development` namespace. Signed RC4-to-RC5 installation, Sparkle replacement, helper registration/approval, preference persistence, and soak checks remain post-publication evidence and are not inferred from workflow success.
...
- owner installation validation confirms that Sparkle detected, installed, and relaunched the signed `v0.19.0-rc.2` update from `v0.19.0-rc.1`; the relaunch exposed an unintended German locale caused by the ambiguous legacy language preference
- owner installed-candidate evidence confirms the next Sparkle check installed and relaunched signed Developer ID `v0.19.0-rc.3`, migrated the legacy English sentinel to `user_locale_preference_v2=system`, and resolved against the current `en-US` macOS application language; the app also follows later Settings language changes, while a newly selected explicit language still needs one owner quit/relaunch persistence check
...
- public default-channel GUI appcast and stable CLI metadata point to signed `v0.19.1` artifacts, while the `beta` appcast item and `latest-rc.json` remain on `v0.19.0-rc.7`.
- owner installation validation confirms `/Applications/Helm.app` reached signed Developer ID builds `0.19.0-rc.2` and then `0.19.0-rc.3` through Sparkle; these are two successive owner installation/update-path validations, not broad participant validation.
...
- approved internal product-design planning track: **Project WOW** defines the base Helm first-run value loop and product allocation at `docs/app-design/PROJECT_WOW.md`; its v0.18 machine contracts, bootstrap audit, CLI/TUI contract, native prototype, state matrix, and research protocol are complete planning artifacts, and staged implementation/validation follows in `0.19.x`-`0.22.x`; the first runtime checkpoint now projects the existing authoritative manager/task snapshots into a schema-aligned, local-only Environment Brief without changing onboarding presentation or enabling mutation/network behavior
- approved internal whole-app design track: the **Native Mac Experience initiative** at `docs/app-design/NATIVE_MACOS_EXPERIENCE.md` now uses Original Wayfinder as its production direction: unified status-item popover plus Dashboard, Dashboard/Plan/Library/Activity workspaces, persistent contextual Environment, and a truthful shared Course Indicator state projection; the v0.18 audit/prototypes remain historical input, while production implementation is staged across `0.19.x`-`0.22.x`
- delivered Original Wayfinder implementation baseline in `v0.19.0`: one deterministic revisioned projection selects approval, failed/interrupted, active, actionable-environment, updates-ready, refreshing/unavailable, or healthy truth for Dashboard, popover, and status-item routing; determinate Course Indicator progress accepts only validated backend-owned completed/total values; stable Dashboard/Plan/Library/Activity/Environment deep links bridge to legacy sections during migration; the existing content host runs in a restored, resizable Dashboard window with `1024x640` minimum sizing; the production shell presents Dashboard, Plan, Library, and Activity as native sidebar peers, keeps Environment persistent at the sidebar foot, and renders a reduced-motion-aware Course Indicator from that shared projection; Dashboard uses a native toolbar for sidebar, contextual inspector, search, refresh, and Upgrade All controls plus app commands for Command-1, Command-F, Command-R, workspace routing, and sidebar/inspector visibility; Command-Comma, the popover, and Dashboard sidebar converge on one native Settings scene with General, Updates, Sources, CLI, and Support panes, pane selection avoids publishing during active SwiftUI view updates, pane title keys are locale-validated, unavailable XPC refresh requests fail closed, and popover reopening does not resubmit in-flight refresh work; either status-item click toggles the same live-localized popover while Dashboard is closed, primary click focuses Dashboard while it is open, and secondary click keeps the popover available alongside it; the one-time AppKit right-click menu is removed, Refresh is available in the footer, and Check for Updates is available in About while existing contextual routes retain Dashboard, Upgrade All, Settings, Support, and Quit; the obsolete quick-Settings overlay plus duplicate CLI card are removed; Service Health and diagnostics snapshot copying now live on Dashboard, the legacy in-window Settings destination and compatibility wrapper are removed, duplicate Refresh/Quit controls are absent from Settings, manager-priority restoration lives with Sources policy, and the Ventura AppKit fallback still opens the native Settings scene; a separate Environment Brief projection preserves brief identity, advances revisions only on semantic observation changes, maps manager eligibility/setup/multi-instance/provenance plus detection failures/cancellation into the v1 contract, and distinguishes current from cached coverage; versioned debug-only first-use/current/partial/offline/service-failure fixtures inject at that shared presentation boundary, while persisted legal/discovering/brief presentation state restores against current brief metadata and removes stale manager selection; a debug-only `HELM_ENVIRONMENT_BRIEF_FIRST_RUN=enabled|preview` gate routes status-item first run into a Dashboard-hosted legal/discovery/brief surface, with `preview` allowing fixture review without resetting completed onboarding and `HELM_ENVIRONMENT_BRIEF_APPEARANCE=light|dark` supporting deterministic appearance review; the five-state minimum/expanded light/dark and German/Hungarian/Japanese/+40% text-expansion presentation matrix passed on 2026-08-08 after mapped-coverage and wrapping remediation; owner-assisted accessibility QA found the VoiceOver rotor and bidirectional Full Keyboard Access defect later closed by the tracked-clean `86f4ef23` matrix; the view remains local/read-only and the existing onboarding stays the production default; default first-run route review/switching after the accessibility closure, setup-session/receipt mapping, destination content migration, and v0.22 multi-display/failure validation remain subsequent slices
- current `v0.20.x` development replaces the legacy mini-Dashboard popover with the approved unified Wayfinder composition. Ordinary states retain a fixed `400x458` panel and consume the shared projection for priority, Course semantics, deep links, completed-check freshness, and affected environment domains; originating conditions plus stable entity/focus/domain targets travel with each route, route-stage actions filter Environment to System, Tools, Apps, or Packages, and Needs Review also selects the exact affected manager and names the known finding instead of repeating generic coverage copy. One context row replaces metric/manager/task mini-lists; Dashboard, Library search, Plan, Activity, and Environment own their respective depth. The utility menu uses native Settings, one-shot direct Support-pane routing, Helm update checks, About, and bounded quit confirmation. The old popover search/About/Quit overlays, popover-hosted upgrade sheet, dynamic fitting-size loop, and popover-only derived-state slices are removed. All seven locale catalogs carry the new vocabulary, and focused projection/routing/size tests run on the macOS 13 arm64 target. A debug-only `HELM_WAYFINDER_POPOVER_FIXTURE` seam renders Healthy, Updates Ready, Running, Needs Review, Error, and offline states through the production presenter, bypasses onboarding, and suppresses refresh plus Helm update checks while active; it remains a popover-only seam. The canonical `v0.20-whole-workflow-v1` corpus supplies semantically validated synthetic records for all seven moderated tasks, including the first-run v1 contract chain, and a Debug-only absolute-path selector ignores nonabsolute paths and all Release builds. Task 2 projects its exact 12-update scenario through the shipping Plan, selection/filter behavior, contextual inspector, selected-row risk model, and bounded final confirmation: ten non-OS updates begin selected, the pinned and macOS rows remain visibly excluded, authority ordering is preserved, and the confirmation cannot apply synthetic work. Task 3 projects its exact cached Homebrew and remote Cargo `ripgrep` results through Library, global search, and the package inspector. Homebrew is the existing-authority recommendation, Cargo appears through bounded progressive enrichment, accepted global results navigate deliberately to the exact Library entity, and the Debug-only offline variant keeps cached truth usable while labeling remote work and the network-backed proposal Deferred. The synthetic install review exposes source, origin, network, and authorization requirements but cannot apply work. Task 4 now projects the exact applied-but-unverified npm activity and the App Store source that never started through the shipping Activity destination and contextual inspector. It keeps Apply and Verification separate, presents localized before/after facts and explicit rollback limits, preserves the ordered Retry Verification, unavailable Restore, Keep Change, and redacted-diagnostics choices, and confines allowed recovery choices to read-only fixture review while permitting only synthetic strictly redacted diagnostics to reach the clipboard. Task 5 now projects all ten exact canonical corpus manager records through the shipping Environment grouping and presents exact rustup dual-install provenance in the contextual inspector: the selected research row exposes `.isSelected` accessibility semantics; the user rustup is active and manageable; the system rustup is inactive and policy-blocked; every instance renders an explicit Active or Inactive badge and accessibility value; authorization cannot override policy; and Keep Multiple is a fixture-local reversible acknowledgment rather than a live manager action. A selected missing or invalid dataset now presents localized unavailable Environment and inspector states without production controls; failed, deferred, review, detected, and not-installed source health remain distinct; Hungarian health labels for Healthy, Error, and Not Installed are localized and regression-protected; the inspector localizes live without reselection; and instance disclosures announce expanded or collapsed state. The Plan presentation now uses a Ventura-compatible native `NSOutlineView` grouped by backend authority, with native selection/keyboard behavior, inclusion and vendor-app actions, contextual toolbar manager/search filters, and virtualized scrolling. Plan owns the only bulk-update confirmation: it snapshots the exact reviewed execution order, automatic and interactive eligibility, privilege/reboot risk, and vendor-app handoffs; any drift disables Run Plan. Dashboard's former generic Upgrade All preview now routes to Plan, and notification Upgrade All resets stale Plan filters, selects the current eligible rows, and requests the same Plan-owned confirmation without implicitly enabling macOS updates. Automated coverage spans empty, single-step, and 125-step plans plus fixed exclusions, stale-selection reconciliation, stale-confirmation rejection, partial failure, cancellation, selected risk, hidden-selection preservation, exact Task 3 result/proposal scope, progressive visibility, offline deferral, accepted-result routing, Task 4 record/action order and recovery truth, and Task 5 exact scenario order, all-manager record identity, rustup provenance/policy identity, source-state health, reversible local decision state, explicit invalid-selection unavailability, and fail-closed drift. Selecting the whole-workflow fixture fails closed before XPC/service startup and blocks refresh, detection, database-backed snapshots, updater traffic, and mutation even when the selected file is missing or invalid. Task 2 Plan and the production native-table Task 3 Library/global-search current-host owner matrices are complete. Task 4's recorded owner matrix covers static visual, Full Keyboard Access, VoiceOver, and localization evidence on base `42c64760`; a source-equivalent pre-commit snapshot later recorded as `c1f7daf` has bounded owner evidence for visible German-to-System-Default Inspector refresh without reselection and approximately +40% Activity/Inspector/recovery reachability. The independent-review locale-change regression proves structural inspector identity, representative `@State` and `@StateObject` preservation, representative Boolean presentation-state preservation, and no child appear/disappear transition. It does not mount the production inspector, a real sheet or popover, a `ScrollView`, a keyboard first responder, or VoiceOver focus; continuity for those production cases remains open absent a remediated-head owner or production-integration retest. Task 1's shared ambient projection, affected-manager and Activity routing, fixed sizing, focus handoff, automated evidence, and attributable current-head visual/interaction/Full Keyboard Access/VoiceOver/localization matrix are complete on PR head `3de57a81`; the retained pre-final-source owner observations remain directional evidence only. For Task 5, the `d2351667` owner matrix remains attributable only for unaffected behavior, Full Keyboard Access, locale refresh, localization, and layout observations. Independent review proved that its no-Hungarian-English-fallback and complete-VoiceOver-semantics claims were inaccurate; exact app source `9e3fa24a` now has attributable owner closure evidence for inactive-badge minimum/expanded Light/Dark/+40% layout, selected/inactive VoiceOver semantics, live Hungarian health and shell presentation, live accessibility-title refresh, locale-specific Settings labels, and independent Settings behavior. The `d60aad9b` pass is historical superseded evidence only. Task 6 now projects the canonical disabled launch-at-login preference through shipping General Settings, keeps preference changes in context-owned fixture memory, suppresses `SMAppService` status reads and mutations while research data is selected, and preserves the exact Activity `7001` route plus strictly redacted Copy Diagnostics action; selected invalid or missing data disables the preference instead of falling back to host state. Task 6 owner validation is complete on exact app source `43f15385`; the bounded Task 7 exact-source owner checks on `e87d30d3` include detailed recommendation/progress/receipt/action-state semantics; the tracked-clean `86f4ef23` matrix closes the Environment Brief heading-rotor and bidirectional Full Keyboard Access gate in Issue #388, while the whole-workflow cross-cutting accessibility pass and participant checkpoint remain open.
- Task 6 automated evidence adds four exact contract regressions and brings the Ventura-compatible arm64 app-target gate to 325 passing tests. The owner completed the visual, interaction, accessibility, localization, layout, diagnostics, reversibility, and cross-window focus matrix on exact app source `43f15385` using an isolated arm64 Debug artifact; this is owner QA, not participant validation. Task 7 is implemented; its bounded canonical flow, summary-redaction, and non-persistent relaunch checks pass on app source `bfbea8c6`, and its bounded exact-source owner checks on `e87d30d3` include detailed recommendation/progress/receipt/action-state semantics. The later tracked-clean `86f4ef23` owner matrix closes the former Issue #388 Environment Brief accessibility gate.
- Independent review of Task 6 found that adjacent privileged-helper and CLI Settings controls escaped the fixture safety boundary, and that blocked launch-at-login states were misleading. Current source now prevents privileged-helper `SMAppService` construction, status reads, registration changes, and Login Items routing; blocks CLI-shim status, install, and removal work; presents localized unavailable truth for missing/invalid data and non-Task-6 fixtures; and raises the arm64 app-target gate to 328 tests with focused zero-read/zero-mutation regressions. The `43f15385` owner matrix remains attributable to the canonical Task 6 launch-to-Activity flow on that exact source. A separate isolated owner pass on exact remediated source `c80d362c` closes the unavailable/safety-blocked Settings, helper suppression, CLI fail-closed, minimum/expanded Light/Dark, VoiceOver, and English/Hungarian/Japanese checks. Task 7's bounded exact-source owner checks on `e87d30d3` include detailed recommendation/progress/receipt/action-state semantics; the later `86f4ef23` matrix closes Issue #388, while participant validation and the whole-workflow cross-cutting accessibility pass remain open.
- PR #528 Task 5 remediation source through `890cf4fe` corrects the Hungarian in-progress Refreshing value to `Frissítés folyamatban` and reapplies the hidden AppKit Dashboard and Settings titles used by accessibility whenever Helm's locale changes; the arm64 app-target gate covers 321 tests. The earlier 13:56 HST artifact remains bounded unattributed evidence because it predates `061e9fa2` and has no retained source mapping. A fresh isolated arm64 Debug app built from exact app source `9e3fa24a3ba6f9945939b8781661beea4c3ae77f` and tracked-clean tree `fdfc45a8da5d67e4f6262752b6bb47b91fa7f69f` closes the current-source Task 5 owner checks; its executable and `Helm.debug.dylib` SHA-256 values are `3dd0cf112f0c2703974d2d3dded7bf44e08c5b491e5cd6634f2b00e14696a83f` and `a71e6ea708f79582ed5920f18854548857f69a0a1817901b8dab01f08ee1fe36`. Dashboard retained full-screen Space association discovered during the pass is tracked separately in issue `#529` and did not affect Task 5 sign-off.
- Task 1's automated evidence boundary is provider/projection/policy/request-state unit coverage plus a successful full app-target build. No automated test drives `HelmOverviewState.applyResearchAmbientHealthRuntimeState`, `ControlCenterContext.navigate(to:)`, or the SwiftUI keyboard/VoiceOver focus transfer; those production-wiring behaviors instead have attributable current-host owner evidence on PR head `3de57a81`.
- current `v0.20.x` package-version state stores missing versions without localized placeholder text. Library, Inspector, and global search resolve the active locale only at presentation time, while semantic package snapshots and update-notification fingerprints normalize legacy localized placeholders so a language change cannot masquerade as newly available software.
- current Task 1 ambient-health projection consumes the canonical whole-workflow dataset through one shared status-item, fixed-popover, Dashboard-hero, and sidebar Course presentation. It identifies the exact applied-but-unverified `prettier` result, reports eight of ten sources available with one failed source needing review and current/cached/review/error route semantics, carries fixed checked freshness, routes the affected System and Packages stages to exact MacPorts and npm managers, and sends Review Recovery to Activity selection `7001` with the inspector revealed plus keyboard/VoiceOver focus requested. Valid and selected-invalid research presentations use the ordinary `400x458` footprint even on an incomplete-onboarding profile; invalid corpus selection shares unavailable ambient truth and reports the service disconnected. The menu-bar tooltip and accessibility label are reapplied when Helm's selected locale changes. The projector rejects identity, safety, scenario, coverage, failed-source-record, and activity drift; the selected fixture still blocks service startup, refresh, detection, persisted snapshots, updater traffic, and mutation. Automated arm64 contract/unit evidence and the full app-target build are complete; current-host owner validation on PR head `3de57a81` covers production wiring, exact routing/focus, valid and unavailable shared state, fixed sizing, live localization, and +40% expansion. The 13:02 HST owner build predates final source changes and has no source-equivalence mapping, so its observations are retained only as directional evidence.
- current Slice 20.3 production Library results carry an additive version 1 provenance object from Rust through FFI/XPC JSON into Swift. The contract distinguishes a result's logical origin from its discovery source, so an item discovered by a manager search and later read from the search cache is labeled Cached rather than Remote while retaining its manager-search query; the discovery field does not claim that every manager search used network access. Installed and outdated manager snapshots are Local, including their persisted service representation. Swift loss-isolates nested decoding and validates schema version, exact canonical source-manager/query values, endpoint-specific origin/discovery rules, and allowed field combinations before showing the localized origin; older payloads remain decodable, and unknown, malformed, or inconsistent provenance suppresses only the label. Exact Library manager selection also remains authoritative in the inspector before persisted preference fallback. The inconsistent adapter cache timestamp is intentionally not exposed, and no SQLite migration is required. Production and Task 3 fixture rows now share one Ventura-compatible native `NSTableView` with column headers, native single selection and keyboard/type navigation, horizontal fallback, context commands, localized icon-action labels, and exact consolidated-member selection for inspector/action routing. The row projection retains status, pin, restart, current/latest version, manager, provenance, and recommendation facts, while accepted global results select, reveal, and focus the exact native row. Automated arm64 macOS tests cover exact member selection and action identity. Owner QA on merged `dev` revision `27e8cc213f001e3f455471bceaefe4751dd194e9` passed the minimum/expanded Light/Dark visual matrix, exact selection and accepted-result focus, Full Keyboard Access, VoiceOver, context commands, type selection, horizontal fallback, and localized icon-action labels. Production remote search now binds submissions to normalized query generations and carries an explicit interactive or description request purpose through XPC/FFI into Rust in-flight deduplication, so package-description work cannot share cancellation ownership with an interactive session. Superseding input cancels only interactive task IDs owned by the prior generation, including late stale callbacks, through a dedicated local-runtime path with the existing 500 ms graceful near-completion window; verified already-terminal Search races are benign, while real cancellation failures remain user-visible. Snapshot reconciliation retires explicitly owned task IDs on terminal status or after two consecutive authoritative snapshot absences without adopting unrelated work. The automated 20,000-row gate is complete: nonempty manager-search results now remain on the invalidation-aware local index through a bounded identity-preserving overlay, and production reuses the same semantic row-projection cache measured by the harness. Cold indexed derivation is `132.928/135.458 ms` median/p95, cold query-to-first-visible p95 is `195.415 ms`, warm indexed search is `22.365/24.332 ms`, and warm production query-to-first-visible p95 is `30.273 ms`. The scroll run keeps 117 of 120 samples within `16.7 ms`, records a `43.616 ms` worst sample, and retains 13 row views. The unique-artifact, host-readiness-aware arm64 harness and later compositor limitation are recorded in `docs/validation/v0.20-library-performance.md`.
- completed RC7 connectivity checkpoint on `dev`: `NWPathMonitor` supplies a three-state unknown/available/unavailable signal across Swift, XPC, FFI, and the shared Rust adapter runtime. An initial refresh waits for a known path; an offline refresh still reconciles local installed-package snapshots but skips remote outdated and catalog tasks before persistence; local/cache browsing, uninstall, pinning, Settings, diagnostics, and refresh remain usable; remote search, descriptions, installs, upgrades, manager downloads/updates, Plan execution, and Helm's Sparkle checks defer without creating failed tasks. A request already in flight when the path is lost retains its actual terminal result. Reconnection resumes at most one deferred full refresh, and non-GUI clients retain their previous behavior until they explicitly provide network state. Owner-assisted QA from merged `dev` revision `f276e2528312c9fcc1431edcb420a08538dc0168` passed online-to-offline transition, local-only refresh, cached Library/Plan usability, network-action gating, offline relaunch, and one coalesced refresh plus deferred Helm information check after reconnection; detailed evidence is recorded in `docs/validation/v0.19.0-rc.7-readiness.md`.
- Task 7 status: the final canonical workflow projection now runs inside the shipping first-run host only behind the paired Debug-only whole-workflow and Environment Brief preview gates. It presents the exact partial brief, reviewed no-network/no-privilege `mise` shell recommendation, verified synthetic progress, Action Receipt, and strict aggregate-only summary. Stage transitions remain in view memory; canonical or record-order drift and missing/invalid paired-preview data fail closed; and leaving the research flow never persists onboarding completion or starts service, database, network, updater, authorization, detection, or mutation work. Focused arm64 projection, transition, paired-gate completion-policy, redaction, drift, all-locale parity, and manager-placeholder regressions pass. The bounded canonical-flow, copied-summary, and same-database relaunch checks pass on app source `bfbea8c6`; the recorded exact-source owner checks on `e87d30d3` cover direct dismissal from every stage, Light/Dark, scoped VoiceOver stage focus/order/roles, copy-success announcement, live localization, representative +40% expansion, selected missing-corpus unavailability, and detailed recommendation/progress/receipt/action-state semantics. The tracked-clean `86f4ef23` owner matrix closes the Environment Brief heading-rotor, focus-geometry, and bidirectional Full Keyboard Access gate tracked in Issue #388. This is owner QA, not participant validation, and the production route remains development-gated pending separate approval.
- PR #534 source `2c8726a0` implements one stable responsive Environment Brief header subtree, native state-title heading semantics, a separate noninteractive Course Indicator percentage, and title-before-count readiness announcements; responsive/all-locale regressions and the full arm64 UI gate pass with 338 tests. The 2026-08-25 owner observations came from a pre-commit artifact without a retained exact source revision, tracked-clean tree, or source-blob equivalence mapping, so they remain bounded unattributed evidence and did not close Issue #388. The later tracked-clean `86f4ef23` rerun supplies the required attributable exact-source rotor, focus-geometry, bidirectional Full Keyboard Access, and wide/vertical layout evidence.
- PR #535 code source `410abbf9` keeps the Course Indicator noninteractive while composing its title and zero-fraction-digit percentage in Helm's selected locale; the all-locale regression verifies both announcement order and the locale-specific percent token, and the arm64 UI gate passes 339 tests. Independent review proved that the retained `8964cef7`/`72b35019` owner artifact was compiled from dirty pre-commit objects before `4b465342` existed. Its inputs appear source-equivalent to `4b465342`, but it is bounded pre-commit evidence rather than tracked-clean exact-source sign-off. The later clean-source `86f4ef23`/`0597de3a` matrix closes Issue #388 and the combined Task 7 accessibility item.
...
- `v0.17.12` moves bulk and scoped upgrade authority sequencing into Rust/FFI/XPC; every submitted task reaches a terminal state before the next phase is scheduled, and scoped-workflow cancellation prevents later-phase submission while preserving individual task cancellation and diagnostics. Current in-process scoped selected-Plan FFI execution additionally waits for every submitted task's ordered domain-response persistence turn to finish before workflow completion becomes observable; the completion receipt denotes turn completion even when an underlying persistence error was logged. Legacy external-coordinator workflows retain terminal-only wait semantics.
...
- active `0.19.x` manager/updater integration: standard application roots are inventoried for embedded Sparkle frameworks; Helm is excluded by bundle identifier; static HTTPS appcasts provide conservative outdated visibility through bounded concurrent checks with per-feed failure isolation; third-party Sparkle actions open the validated vendor app so its own updater retains installation authority; eligible direct-channel Helm builds expose Sparkle-backed scheduled checks plus a persisted, default-off beta/RC opt-in; channel-aware publication requires stable tags to remain published non-prerelease GitHub releases owning `releases/latest`, requires RC tags to remain published GitHub prereleases that never own latest, preserves simultaneous stable and RC lines across appcast/CLI metadata, merges appcast changes against current `main`, and rejects per-channel rollback; npm multi-install normalization and MAS refresh handling are hardened, and npm search returns an empty result without process execution for trimmed queries shorter than its two-character minimum; the minimum supported macOS release moves to macOS 13 Ventura.
- current updater-plan integration keeps scheduled direct-channel Helm checks information-only and projects discovered Helm updates into Helm's package and plan surfaces. Local Helm and third-party Sparkle rows are rebuilt independently of the service preview, so an empty or temporarily unavailable backend plan cannot leave Library/sidebar availability visible while Plan is blank. A persisted, default-off setting may include Helm in Upgrade All; when enabled, all service/core work reaches terminal state before Sparkle presents its confirmation and relaunch flow. Third-party Sparkle updates remain interactive steps and are never counted or dispatched as automatic bulk work; Plan, Library, popover, and inspector actions consistently open the vendor app to finish updating. Plan presents each Mac App Store update as an individual app, checks every newly available step by default, lets the user exclude any row before execution, and can select or deselect the currently visible filtered rows without changing hidden selections; privilege and restart risk flags derive from those selected rows, with ordinary Homebrew work classified as unprivileged. The XPC/Rust boundary accepts the complete reviewed backend-step snapshot, re-previews immediately before workflow registration, and fails closed on missing, reordered, duplicate, or semantically changed steps; execution uses only that validated snapshot. A default-on Notifications preference gates local alerts; newly observed updates notify only while Helm's interfaces are hidden and no upgrade Plan is running, and provide Review Plan plus an eligibility-gated Upgrade All action that opens the existing confirmation sheet rather than bypassing plan policy. Active backend Plan execution removes pending update-availability alerts, consumes changing update fingerprints, and remains suppressed through the first accepted post-execution outdated-package snapshot; a reviewed Helm-only Plan start performs the same one-shot alert removal and fingerprint consumption without treating unrelated manual Sparkle checks as Plan execution. Normal automatic Plan completion preserves any remaining interactive steps in an in-app handoff and posts a system notification, deep-linked to Plan, when Helm's interfaces are hidden. Routine all-tasks-complete alerts are removed, while actionable timeout decisions remain. Fleet remains PKG/MDM-controlled; this slice establishes reusable policy and sequencing only.
- current native-shell polish uses Helm's bundled anchor app icon in the Dashboard sidebar brand, keeps the selected Plan badge legible, reports all implemented detected managers in Service Health regardless of enablement, separates disabled detected managers from supported managers that are not installed, gives Manager Health cards one consistent two-line title region across one-line and wrapped manager names, gives the Settings window a compact default and bounded layout, and allows Settings to join the active Space alongside full-screen apps. Dashboard and Settings now place their navigation in top-level semantic `NavigationSplitView` sidebar columns: macOS owns title-bar integration, resizing, collapse state, divider, inactive-window response, and standard toolbar toggles, while redundant visible window titles are hidden and Helm supplies only a restrained Original Wayfinder surface from pale blue at the upper-left to near-white at the lower-right plus its iconography, route hierarchy, badges, persistent Environment and Settings actions, and status footer. The sidebar surface extends through the full-size title-bar safe area beneath the traffic lights and native toggle. Dashboard keeps its trailing inspector in an in-detail `HSplitView` on every supported release, so inspector toggling consumes detail space without changing the outer window frame or sidebar position, and no supported release wraps the navigation sidebar in an outer AppKit split controller. AppKit remains the single authority for window geometry: Dashboard enforces a `1024x640` minimum, Settings enforces `600x420`, restored frames are moved fully onscreen, and the development-gated Environment Brief uses a fixed, non-resizable `960x600` first-run frame before restoring the normal Dashboard. Contextual Dashboard toolbar actions and the Plan manager filter use native leading placement; the native search field and Details toggle occupy separate trailing items using a hidden principal alignment anchor. Tahoe gives search an independent native glass surface, while older releases retain the existing search appearance. Dashboard window movement is title-bar-only; the former content-wide drag mode and manager-hover suppression state are removed. The Dashboard application-menu sidebar command remains synchronized with native column visibility. Plan retains its Ventura-compatible native authority-grouped outline, keyboard selection, and virtualized scrolling while using a rounded Wayfinder surface, visible authority headings, roomier rows, manager subtitles, and Wayfinder selection treatment. Support Helm remains a clean full-height action beside its two-button peer stack. The shared semantic status system now distinguishes green Healthy, blue Running, sea-glass teal Updates Ready, amber/orange Needs Review, red failures, and neutral gray unavailable/deferred states across Dashboard, Plan, Library, Environment, Settings, popover, first run, inspectors, notifications, and the menu-bar indicator. Compact status badges keep canonical labels on one line, and Environment rows needing review expose their specific finding, including multiple installs, without requiring inspector discovery. Routine update availability no longer degrades aggregate Service Health; the Plan badge is the Dashboard sidebar's update signal, so update-only state does not duplicate a review finding in the footer, while approval, findings, service problems, running work, and failures retain their contextual footer routes. Detected manager cards retain within-authority priority dragging, while Not Installed cards no longer present or initiate the reorder gesture. The development-gated Environment Brief keeps its header, system summary, and completion actions anchored while only its software-source list scrolls; the source column extends beside the left-column actions so both columns share one bottom edge, and a tested Management Readiness summary classifies sources into mutually exclusive ready, needs-review, and observed-only counts. The obsolete manual Replay Walkthrough control is removed, while the existing production first-run onboarding route remains unchanged until the separately reviewed production first-run route is approved; the Issue #388 accessibility gate is closed. Current-host appearance/accessibility review and Ventura-compatible CI evidence for the native-sidebar slice are tracked in `docs/validation/v0.20-native-sidebar-readiness.md`.
- Plan presentation now renders the Ventura-compatible native authority-grouped outline as a vertical Wayfinder card list. It removes the spreadsheet header and grid while retaining virtualized scrolling, full-row selection, manager subtitles, trailing status, leading inclusion controls, vendor actions, keyboard toggling, contextual inspector routing, and disclosure choices for sections temporarily removed by filtering.
- current privileged-execution foundation requires every elevated process request to carry one stable `PrivilegedOperation` identity and rejects mismatched/untyped elevation state before spawn. A trusted absolute executor can be configured once inside the service process to receive that identity plus the resolved structured command; inherited environment cannot select it, and absent that seam the existing `sudo -A` askpass implementation remains active. `developer_id` app bundles embed a dual-mode `HelmPrivilegedHelper` executable plus its Service Management launch-daemon plist, and release automation explicitly signs and verifies the nested helper. Non-direct channels strip those artifacts and do not present native-helper controls. General Settings now offers explicit register/unregister actions only when the direct-channel artifacts are present, maps every `SMAppService.Status`, opens Apple's Login Items pane when approval is required, refreshes state when Helm becomes active, and fails visibly on missing artifacts or operation errors. The helper accepts only its exact signed client whose immediate parent is the exact signed Helm service, revalidates every request without a shell, limits execution to three fixed Apple `softwareupdate` shapes, bounds captured output, and terminates associated work when the caller disconnects. Registration does not activate execution: the helper is not configured as the runtime executor, so privileged updates continue through `sudo -A` askpass. Mac App Store and all MacPorts mutations remain pinned to askpass pending bounded designs for their unsigned/dynamic command surfaces. Runtime activation, signed installed-build registration/approval/denial/cancellation QA, and a bounded MAS replacement path remain incomplete; `docs/architecture/PRIVILEGED_EXECUTION.md` records the completion gates.
...
- 0.17.x — Diagnostics & Logging (**stable released on `main`**, RC lineage `v0.17.0-rc.1` through `v0.17.0-rc.5`)
  ...
  - delivered: post-`rc.3` control-center/popover interaction hardening: primary status-item clicks focus Dashboard while it is visible, secondary clicks keep the popover available alongside Dashboard, and popover header/metric cards deep-link into Dashboard sections.
  - delivered: post-`rc.3` manager inspection/prioritization follow-up: manager inspector now surfaces prioritized install-method metadata with recommended/preferred tags, displays all discovered executable paths with active-path emphasis, and managers are now priority-ordered with intra-authority drag reordering plus restore-default support in advanced settings.
  ...
  - delivered: post-`v0.17.5` release-process hardening phase 2 (on `dev`): `release-cli-direct.yml` and `release-macos-dmg.yml` now treat open fallback publish PRs as explicit follow-up-required outcomes (non-red) instead of terminal failures, including full JSON snapshots where GitHub reports nullable `mergedAt`; hard failures remain for build/signing/notarization/upload/PR-creation faults, and both workflows emit publication summaries (`Artifacts uploaded`, `Publish PR opened`, `Main metadata synced`) with rerun guidance when metadata sync remains pending.
  ...
  - delivered: post-`v0.17.5` release-process hardening phase 6 (on `dev`): `runbook.sh tag` retains strict current-`main` preflight by default and adds an explicit isolated-worktree mode that refuses local changes, freshly fetches `origin`, and requires the release worktree `HEAD` to equal `origin/main` exactly before creating or pushing a tag.
  ...
  - delivered: post-`v0.17.7` managers inspector interaction hardening follow-up (on `dev`): selected-row highlight overlays in Managers now opt out of hit testing so row toggles/buttons remain clickable while selected, and manager `Update`/`Uninstall`/`View Packages` actions now live in the inspector as icon actions consistent with package inspector controls.
  - delivered: post-`v0.17.7` manager install-flow UX follow-up (on `dev`): non-installed managers in inspector now show an install action (instead of disabled `View Packages`), manager install actions now open a confirmation sheet with install-method selection plus collapsible advanced timeout options (hard + idle), and inspector inline install-method/timeout controls were removed so those options are scoped to install confirmation.
  - delivered: post-`v0.17.7` manager install verification follow-up (on `dev`): successful manager installs now transition the manager into a `Verifying` operation state, trigger manager-scoped detection/refresh through service + FFI (`helm_trigger_detection_for_manager`), treat verifying managers as effectively not installed for action gating, and clear verification only after a new detection task for that manager reaches terminal state (with full-detection fallback on scoped-trigger failure/timeout).
  - delivered: post-`v0.17.7` rustup install-source follow-up (on `dev`): inspector install flow now exposes rustup-init install source options (`Official Download` default or user-selected existing `rustup-init` binary path), service/FFI manager-install APIs now accept optional install-options payloads, CLI manager install now supports rustup source/path flags (`--rustup-install-source`, `--rustup-binary-path`) through the same planner model, and rustup manager install now supports official bootstrap download execution when no local `rustup-init` binary is present.
  - delivered: post-`v0.17.7` shared onboarding-state follow-up (on `dev`): GUI onboarding/license acceptance now synchronizes through the shared SQLite `app_settings` onboarding keys used by `helm-cli` (via service/FFI), while retaining `UserDefaults` as a local mirror; this eliminates GUI-complete/CLI-prompting mismatches.
  - delivered: post-`v0.17.7` Rust build-toolchain resilience follow-up (on `dev`): Xcode Rust build orchestration now validates the selected rustup toolchain before build, auto-repairs corrupted toolchains via uninstall+reinstall when enabled, scopes rust-target discovery/installation to the same toolchain for deterministic behavior, and clears unsupported `LC_ALL=C.UTF-8` in script context to reduce noisy locale warnings.
  - delivered: post-`v0.17.7` task-progress monitoring + live-feedback follow-up (on `dev`): process idle-timeout activity now resets on stdout/stderr output and sampled process CPU progress (macOS via `proc_pid_rusage`, Linux via `/proc/<pid>/stat`), rustup self-uninstall now uses a 15-minute hard timeout with a 180-second idle timeout, and Control Center running-task dropdown output now renders both Helm task logs and task stdout/stderr streams together.
  - delivered: post-`v0.17.7` Homebrew long-running task timeout follow-up (on `dev`): Homebrew lifecycle requests (`install`/`uninstall`/`upgrade`/`cleanup`) now default to a 4-hour hard timeout with a 30-minute idle timeout, and process execution now applies bounded hard-timeout extensions for active mutating tasks (`install`/`uninstall`/`upgrade`) so sustained-progress jobs are less likely to be terminated prematurely while preserving an absolute cap.
  - delivered: post-`v0.17.7` task-timeout notification follow-up (on `dev`): hard-timeout handling now publishes actionable timeout prompts through core/FFI/XPC (wait or stop) before forced termination, and GUI now posts macOS notifications for pending timeout decisions plus "all tasks complete" transitions only when neither Control Center nor menu-bar popover is active.
  - delivered: `0.18.x` doctor/repair persistence and execution follow-up (on `dev`): migration 17 makes SQLite canonical for scoped scan generations, finding lifecycle, versioned repair knowledge, provenance/audit state, local overrides, and repair history, while migration 19 persists knowledge recommendation rank; bundled knowledge is imported at store startup through the same strict RFC 8785/SHA-256 envelope contract used for external imports; planning at CLI and FFI/XPC surfaces resolves effective local knowledge, localized content keys, and deterministic recommendation rank against a compiled typed-action registry; hostile imports, trust escalation, protected rebinding, policy weakening, equivocation, and stale scan completion fail closed; repair apply revalidates active findings, records CLI/FFI history, and verifies outcomes through follow-up scans; scoped scan lifecycle coverage includes full, partial, failed, and cancelled completion.
  - delivered: Homebrew execution-domain locking follow-up (on `dev`): Homebrew formula and cask tasks now share one process-wide core orchestration lock across adapter runtimes, preventing concurrent Helm operations against the same Homebrew installation while preserving existing authority-phase scheduling; cancellation retains the shared lease until active adapter work exits, timed-out request/response tasks are cancelled before retry/return, formula and cask lock-conflict signatures are classified into structured diagnostics, GUI lock-conflict guidance is localized, and transient doctor findings expire after one hour.
  - delivered: post-`v0.17.7` manager post-install setup gating follow-up (on `dev`): doctor now emits `post_install_setup_required` findings for installed `rustup`/`mise`/`asdf` instances missing required shell setup, repair planning now exposes `apply_post_install_setup_defaults`, FFI/runtime manager enablement blocks when setup is incomplete, GUI manager inspector now surfaces `Finish <Manager> Setup` guidance with copyable manager-specific commands and verify flow, install confirmation now includes optional `Complete post-install setup automatically`, and CLI doctor-repair apply now handles setup-default automation with immediate manager detection follow-up.
  - delivered: post-`v0.17.7` manager-provenance phase 1 groundwork (on `dev`): SQLite schema now persists per-manager install instances (`manager_install_instances`, migration v9) with stable identity (`identity_kind` + `identity_value` + deterministic `instance_id`), alias-path dedupe for symlink continuity, persisted provenance/confidence/explainability and confidence-to-automation strategy fields, runtime detection now materializes instance records alongside legacy single-path detection state, and phase-1 rustup-first adapter scoring (with bounded/lazy external Homebrew evidence) established the baseline for later per-manager rollout; `helm-cli` now exposes install-instance provenance surfaces via `helm managers instances [<manager-id>]` plus `managers list/show` active-provenance summaries.
  - delivered: manager-provenance phase 2 confidence/margin hardening (on `dev`): install-instance persistence now records provenance decision margin (`decision_margin`, migration v10) alongside confidence and competing provenance, provenance classification now routes through adapter-level spec hooks (initially rustup-first, now expanded across manager adapters), confidence thresholds/automation mapping constants are now centralized in shared core policy module `provenance_policy.rs`, rustup scoring now covers default/custom cargo-home paths plus asdf/mise-managed layouts with optional bounded `pkgutil` ownership evidence for ambiguous managed-prefix installs, bounded external provenance probes now emit structured cache/timeout/success diagnostics, boundary coverage now includes confidence/automation policy assertions and fail-closed external-probe tests (including timeout-bounded hung-probe responsiveness checks), identity tests now explicitly cover fallback-hash continuity and canonical-path fallback behavior (with non-Unix fallback-reset coverage), CLI manager summaries/instance inspection now surface active decision margin to support operator trust and automation-policy tuning, and operator guidance now lives at `docs/operations/MANAGER_PROVENANCE_TROUBLESHOOTING.md`.
  - delivered: manager-provenance API parity follow-up (on `dev`): FFI/XPC manager-status payloads now include both per-manager install-instance lists and active-install-instance provenance/confidence/margin/automation/strategy/explainability summary fields so GUI surfaces can consume the same provenance trust metadata exposed in CLI; manager inspector now renders multi-instance provenance cards (path + confidence/margin + explanation + competing provenance) for ambiguous/multi-install diagnosis; `helm managers install` now supports a one-off `--method <method-id>` override without mutating persisted install-method preferences; install-method resolution now supports interactive TTY fallback for ambiguous method sets while returning deterministic non-interactive `--method` requirement errors in machine/script contexts; and TUI manager install now resolves the same install-method policy path (including supported-method fallback and ambiguous-selection gating that requires choosing manager install method in-app via `m` before install submission).
  - delivered: manager-provenance install-flow + calibration follow-up (on `dev`): GUI manager install now opens an install-method selection sheet before submission (recommendation-rank ordering, Helm-supported method filtering, and persisted selected-method handoff before task launch), install-method metadata now carries explicit recommendation rank/reason + policy-tag fields for managed-environment gating follow-through, and rustup provenance calibration coverage now includes ambiguous `/usr/local` mixed-environment fixtures that validate confidence resolution with bounded external evidence (`brew --prefix rustup`, `pkgutil --file-info`) for Homebrew-vs-System ownership decisions.
  - delivered: manager-provenance managed-policy gating follow-up (on `dev`): install-method policy resolution now uses a shared managed-environment context (`HELM_MANAGED_INSTALL_METHOD_POLICY`, `HELM_MANAGED_INSTALL_METHOD_POLICY_ALLOW_RESTRICTED`) so CLI/TUI install-method list/cycle surfaces only expose policy-allowed methods, explicit install-method selection rejects policy-blocked options, and GUI install-method pickers/inspector menus now disable policy-blocked choices for parity while surfacing recommendation/policy badges (`Recommended`/`Preferred`/`Managed Restricted`/`Blocked`) and disabling install submission when no policy-allowed method remains.
  - delivered: manager-provenance managed-automation policy follow-up (on `dev`): manager automation now supports a managed-policy ceiling override (`HELM_MANAGED_AUTOMATION_POLICY=automatic|needs_confirmation|read_only`, defaulting to `needs_confirmation` when managed-install policy is active) and applies it consistently across CLI + FFI manager status/instance projections and lifecycle planning so managed environments can conservatively clamp mutation automation without mutating persisted provenance records.
  - delivered: manager-provenance phase 3 rustup uninstall safety slice (on `dev`): `helm managers uninstall` now supports structured blast-radius preview (`--preview`) and explicit confirmation (`--yes`), rustup uninstall routing now prefers active install-instance provenance strategy (homebrew vs rustup-self) instead of install-method preference, unknown rustup provenance now supports preview without override but still requires explicit override (`--allow-unknown-provenance`) before mutation, and preview payloads now include files/directories/secondary-effects plus confidence/automation explainability (including standardized summary lines/unknown-override/read-only flags across GUI and CLI surfaces). Shared manager uninstall-preview model/build logic now lives in `helm-core` (`uninstall_preview.rs`) and is consumed by both CLI and FFI/UI paths to keep blast-radius/provenance reasoning consistent across surfaces. Package-manager uninstall preview now reuses the same shared core preview module and `helm packages uninstall` supports `--preview` plus threshold-gated `--yes` confirmation with structured blast-radius output in text/JSON. Rustup update routing now also resolves from active provenance strategy (homebrew vs rustup-self) and blocks read-only/ambiguous provenance paths instead of silently falling back to selected install-method defaults. Rustup remediation strategy outputs are now provenance-specific (`homebrew_formula`/`rustup_self` for known ownership, `interactive_prompt` for unknown, `read_only` for system/enterprise/nix) so blocked/ambiguous paths surface deterministic guidance. Multi-install routing coverage now explicitly verifies active-instance precedence for rustup update/uninstall planning. TUI manager uninstall confirmation now surfaces preview summary fields (strategy/provenance/confidence/blast radius) and supports explicit unknown-provenance override via `Shift+X`; TUI package uninstall confirmation now also includes package uninstall preview summary before destructive execution. GUI manager inspector uninstall now fetches the same backend preview through XPC/FFI, includes preview summary lines in confirmation, blocks destructive execution for read-only automation, and routes explicit unknown-provenance override via uninstall-with-options execution. GUI package uninstall confirmation now also uses the same backend preview through XPC/FFI (inspector + popover quick actions) before submitting destructive package uninstall tasks. Compatibility uninstall routing for currently supported non-rustup managers (`mise`, `mas`) no longer depends on selected install-method preferences, preserving install-method state as install-only intent while broader provenance-first rollout progressed.
  - delivered: manager-provenance phase 4 initial non-rustup routing slice (on `dev`): `helm managers uninstall` now routes `asdf`, `mise`, and `mas` through active install-instance uninstall strategy (homebrew formula vs read-only) rather than compatibility fallback; ambiguous provenance for these managers now follows explicit override policy (preview allowed without override, mutation requires `--allow-unknown-provenance --yes`). `asdf` now has adapter-specific provenance scoring for homebrew vs asdf-managed layouts, and `asdf`/`mise`/`mas` manager update routing now resolves from active install-instance strategy and blocks ambiguous/read-only paths instead of consulting install-method preference. CLI regression coverage now includes provenance-driven install/update/uninstall planner tests for all three managers. Follow-up rollout now includes adapter-specific provenance classification for `npm`/`pnpm`/`yarn`, `pip`/`pipx`/`poetry`, `rubygems`/`bundler` (including version-suffixed executable aliases such as `bundle3.4`), `cargo`/`cargo-binstall` (with cargo-home layout heuristics for `~/.cargo/bin` and optional `CARGO_HOME/bin`), and guarded/system managers (`softwareupdate`, `macports`, `nix_darwin`, `sparkle`, `setapp`, `homebrew_cask`, `docker_desktop`, `podman`, `colima`, `parallels_desktop`, `xcode_command_line_tools`, `rosetta2`, `firmware_updates`). Core provenance dispatch now has adapter-specific coverage for all current manager IDs (no per-adapter `TODO(provenance-spec)` fallback stubs). Lifecycle routing is now broadened beyond `rustup`/`asdf`/`mise`/`mas` for Homebrew-owned manager paths where formula ownership is one-to-one (`pnpm`, `yarn`, `pipx`, `poetry`, `cargo-binstall`, `podman`, `colima`) with the same ambiguous/read-only blocking policy and uninstall preview gating. Additional provenance-first manager routing now supports dynamic Homebrew parent-formula ownership for `npm`, `pip`, `rubygems`, `bundler`, and `cargo` when active install-instance canonical paths resolve to `Cellar/<formula>/...`; unresolved formula ownership now hard-blocks both update and uninstall with explicit diagnostics guidance. Homebrew-routed manager uninstall planning now uses a shared explicit read-only path for read-only automation outcomes (preview allowed, mutation blocked) so read-only previews do not imply Homebrew removals. CLI command-level scripted uninstall tests now verify explicit flag gating (`--yes`, `--allow-unknown-provenance`) and preview-hint messaging for deterministic automation behavior, including rustup multi-install and unknown-provenance command-path smoke coverage (`--preview` happy path + non-preview override hint). Manager-summary selection now normalizes per-manager install-instance ordering before active/fallback selection so CLI summary and instance surfaces remain aligned even when no instance is marked active, with regression tests covering active-first sorting and no-active fallback parity. Install-instance classification coverage now also explicitly verifies ambiguous `/usr/local/bin/*` unknown fallback for remaining runtime managers (`npm`, `pnpm`, `pipx`, `poetry`, `rubygems`, `cargo`), guarded-system non-system paths (`softwareupdate`, `xcode_command_line_tools`, `rosetta2`, `firmware_updates`), and nonstandard `macports`/`nix_darwin`/`homebrew_cask` paths.
  - delivered: manager-provenance FFI/GUI parity follow-up (on `dev`): manager update/uninstall routing in `helm-ffi` now applies the same provenance-first Homebrew strategy model used by CLI for one-to-one Homebrew managers (`asdf`, `mise`, `mas`, `pnpm`, `yarn`, `pipx`, `poetry`, `cargo-binstall`, `podman`, `colima`) and dynamic parent-formula managers (`npm`, `pip`, `rubygems`, `bundler`, `cargo`), including ambiguous-override and read-only blocking behavior; manager-status install-instance ordering/fallback selection now uses `is_active` + `instance_id` for deterministic no-active parity between GUI and CLI provenance summaries, and FFI regression coverage now includes one-to-one + parent-formula manager routing loops plus unresolved-formula/read-only uninstall assertions.
  - delivered: adapter generalization follow-up (on `dev`): install-method and package-search participation policy maps now resolve from shared `helm-core` registry metadata (removing duplicated CLI/FFI maps), provenance score finalization now runs through a shared helper in `install_instances`, external evidence probing now supports generic keyed `brew --prefix <formula>` caching (lazy + timeout-bounded + fail-closed), common Homebrew/rustup lifecycle strategy resolution helpers now live in `helm-core::manager_lifecycle` with CLI/FFI wrappers mapping shared typed outcomes to surface-specific messaging, manager-status payloads now expose core-owned install-method metadata consumed by the GUI inspector install sheet (with local hint fallback) for CLI/FFI/UI parity, manager-install request planning (including rustup install-source validation/routing) now resolves through shared `helm-core::manager_lifecycle::plan_manager_install`, and manager update/uninstall routing now also resolves through shared planner APIs (`plan_manager_update` + `plan_manager_uninstall_route`) for CLI/FFI parity while preserving preview/override/read-only gating behavior.
  - delivered: multi-instance manager attention/override follow-up (on `dev`): manager status now includes shared multi-instance state (`none`/`attention_needed`/`acknowledged`) plus ack/fingerprint diagnostics across CLI + FFI/XPC, multi-instance acknowledgements now persist by manager with deterministic install-set fingerprint invalidation when install instances change, active managed install instance can now be explicitly overridden by instance id (and clears stale acknowledgement) through shared APIs, CLI/TUI now expose `ack`/`clear-ack`/`set-active` controls, and GUI manager inspector now surfaces attention/acknowledged banners with `Keep Multiple`, `Re-evaluate`, and per-instance `Manage This Instance` actions while manager health reflects multi-instance attention state.
  - delivered: manager dependency enablement-guard follow-up (on `dev`): manager provenance dependency mapping is now centralized in core (`manager_dependencies`) and reused by runtime recommendation sorting, manager disable now hard-blocks when enabled dependent managers currently rely on that parent (CLI/TUI + FFI enforcement), GUI Managers toggles now preflight dependency transitions with explicit modals (disable-block acknowledgement listing dependents; enable-child confirmation that also enables the parent manager), and service error localization now includes dependency-block messaging.
  - delivered: post-`v0.17.7` mise provenance calibration follow-up (on `dev`): `mise` install-instance classification now uses a dedicated adapter-specific scorer (instead of Homebrew-only formula heuristics), covering Homebrew/script-installer (`~/.local/bin/mise`), cargo-home, npm-global (`@jdxcode/mise`), MacPorts, Nix, system, and enterprise-style managed prefixes with bounded/lazy optional `brew --prefix mise` + `pkgutil --file-info` disambiguation on ambiguous paths.
  - delivered: post-`v0.17.7` mise lifecycle parity follow-up (on `dev`): `mise` install planning now supports all Helm-supported methods (`scriptInstaller` recommended/default, `homebrew`, `macports`, `cargoInstall`) across CLI/TUI/GUI through shared lifecycle planners; script-installer install source options now include official download (default) and existing binary path; uninstall now supports structured options (`managerOnly` default or explicit `fullCleanup` with required config choice keep/remove), routes through provenance-aware strategy planning, and surfaces blast-radius previews and confirmation gates consistently across CLI, TUI, and GUI/XPC/FFI paths.
  ...
  - delivered: GitHub governance hardening follow-up (on `dev`): per-branch rulesets are now explicit for `main`/`dev`/`docs`/`web` with branch-specific required checks; new `Policy Gate` + `Docs Checks` + `Web Build` workflows enforce branch targeting/scope policy; repository merge settings keep auto-merge/update-branch enabled while delete-branch-on-merge stays off to protect primary branches; blocking ruleset `update` enforcement was removed after protected-ref merge-block diagnostics; CodeQL Advanced Setup now analyzes Actions, JavaScript/TypeScript, Rust, and Swift on pull requests targeting `main`/`dev` and on `main` pushes, with manual dispatch retained and no mixed-in schedule that GitHub can disable for repository inactivity.
...
- The Sparkle apps manager inventories Sparkle bundles under `/Applications` and `~/Applications` and derives conservative outdated state only from static HTTPS appcasts using bounded concurrent checks and an aggregate scan budget. Helm is explicitly excluded and dynamic-feed apps remain inventory-only. Installed-candidate QA confirmed that sandboxed Helm cannot complete arbitrary target-specific `-spks`/`-spki` installer handoffs after downloading an update, so the RC6 candidate routes every third-party Sparkle action to the validated vendor app instead of offering the broken direct path. Outdated third-party Sparkle apps remain excluded from automatic bulk execution and manager lifecycle automation because Sparkle owns each app's confirmation and Helm has no reliable completion contract. A dedicated signed/notarized external-updater boundary and verified completion contract are planned for `0.20.x` rather than weakening Helm's app sandbox.
...
- CLI manager lifecycle mutation coverage now includes `managers install|update|uninstall` with install-method-aware routing for currently supported manager targets (`mise`, `mas`, `rustup`, `homebrew_formula`) and Homebrew dependency guards where required.
- CLI update orchestration now includes `updates preview` and `updates run --yes` with upgrade-step planning over the cached outdated snapshot, including pinned-filter and OS-update guard flags (`--include-pinned`, `--allow-os-updates`).
- CLI task inspection surface now includes `tasks logs` and `tasks output` JSON/human views plus `tasks follow` (polling persisted task logs); `tasks follow` currently enforces a timeout guard to avoid indefinite hangs on non-terminal snapshots.
- CLI task cancellation (`tasks cancel`) now routes through the shared CLI coordinator and supports process-level cancellation for coordinator-owned tasks.
- CLI settings mutation now includes `settings reset <key>` for implemented keys (`safe_mode`, `homebrew_keg_auto_cleanup`, `auto_check_for_updates`, `auto_check_frequency_minutes`) in addition to existing `settings set`.
- CLI command-scoped help now includes nested subcommand forms (for example `helm packages install help`, `helm managers executables help`, `helm help managers executables set`) in addition to top-level command help.
- CLI read-only command polish now includes a panic fix for `helm updates`/`helm updates --json` with no explicit subcommand, and JSON envelope consistency (`schema`/`schema_version`/`generated_at`/`data`) for `search`, `managers show`, `settings list`, and `settings get`; `settings list --json` now includes `auto_check_for_updates` and `auto_check_frequency_minutes`.
- CLI read-only list ergonomics now include `--limit` for `packages list`/`ls`, `updates list` (including top-level `updates` flag-form), and `tasks list` (including top-level `tasks` flag-form), plus `tasks list --status` filtering.
- CLI global verbose diagnostics flags are now implemented (`-v`, `--verbose`), emitting additional execution/coordinator details to `stderr` while keeping `stdout` contract-safe for human and JSON payload modes.
- CLI settings persistence now includes `auto_check_for_updates` and `auto_check_frequency_minutes` (`settings get|set|reset` and `settings list`/`self status` now reflect persisted values).
- CLI `self` namespace is now provenance-aware across supported channels (`direct-script`, `brew`, `macports`, `cargo`, `app-bundle-shim`, `managed`, `unknown`): `self status`/`self check`/`self update` now enforce channel policy, provide channel-specific recommended actions, and perform direct binary self-update only when policy allows.
- CLI `self auto-check` controls are now implemented with persisted settings (`self auto-check status|enable|disable|frequency <minutes>`), with nested help/completion coverage and `settings` parity via `auto_check_for_updates` + `auto_check_frequency_minutes`; scheduled coordinator ticks now persist `auto_check_last_checked_unix`.
- CLI manager enable/disable now performs best-effort cancellation of manager-scoped queued/running tasks through the CLI coordinator when disabling a manager (with cancellation diagnostics in JSON/human output).
- CLI mutation JSON output consistency now uses the standard envelope (`schema`, `schema_version`, `generated_at`, `data`) across manager enable/disable, settings set/reset, manager detection wait payloads, and shared manager-result emitters.
- CLI diagnostics command namespace now includes `diagnostics summary`, `diagnostics task`, `diagnostics manager`, and `diagnostics export` with JSON/human output and structured export file writing (`--path`).
- CLI completion command now includes `completion bash|zsh|fish` shell-script generation.
- CLI coordinator routing is now active for single-task mutation flows (`packages install|uninstall|upgrade`, `managers detect <id>`, `managers install|update|uninstall`) with true `--detach` submission and task-id return.
...
- CLI audit-remediation follow-up delivered on `dev`: `helm doctor` now maps to dedicated doctor workflows (`scan`, `repair`) with no-argument default `scan` behavior, diagnostics provenance/export remain under `helm diagnostics ...`, self-update `--force` is now constrained to `direct-script` installs (channel-managed installs remain blocked), coordinator auto-check ticks now require direct-script marker policy before network checks, and direct install/update URL fetch paths now enforce allowlisted HTTPS hosts with explicit timeout policy (plus opt-in local `file://` override for test workflows).
...
- CI runtime maintenance now pins every official Node-based GitHub Action reference currently used by the repository to immutable Node.js 24-compatible releases (`actions/checkout` v7.0.1, `actions/setup-node` v5.0.0, `actions/cache` v5.1.0, `actions/upload-artifact` v6.0.0, `actions/dependency-review-action` v5.0.0, and `github/codeql-action` v4.37.3); the CI toolchain contract now enforces those pins repository-wide, and the upgraded `actions/*` set requires Actions Runner 2.327.1 or newer.
...
0.13.x through the `v0.19.1` maintenance checkpoint are complete on `main`; `v0.18.0` remains withdrawn, `v0.18.1` is its corrective successor, `v0.18.2` is the final v0.18 containment release, and `v0.19.1` is the current stable release.
