...
Current documentation baseline: **0.18.2 remains the latest public stable release on `main`**, and **0.19.0-rc.6 is the published opt-in prerelease**; `v0.18.0` remains withdrawn because of a critical SQLite migration defect. `v0.18.2` is the final v0.18 containment release and includes migration-safety hardening plus the closed Project WOW and Native Mac Experience design-definition artifacts that preceded active v0.19 implementation.

Implementation baseline: **published `0.19.0-rc.6` with a prepared `0.19.0-rc.7` source candidate on `dev`**, retaining the RC6 Dashboard, Settings, updater, notification, Plan-selection, manager-compatibility, and privileged-operation foundations. The RC7 source candidate adds clarified Dashboard manager semantics, filtered Plan bulk selection, detected-manager-only priority dragging, a simplified Settings detail surface, macOS network-path monitoring, a first-class localized offline Wayfinder state, pre-submission deferral for network-dependent core work, local-only inventory refresh while offline, action gating, deferred Helm update checks, and one coalesced refresh after reconnection. Focused owner-assisted online/offline/relaunch/recovery QA and the full repository quality gate completed successfully on 2026-08-15. RC7 is not tagged or published, and public prerelease metadata remains on RC6 while stable GitHub/latest, appcast, and CLI metadata remains on `v0.18.2`. Installed-candidate replacement, Settings, preference-persistence, and third-party vendor-update checks remain explicit owner evidence rather than inferred publication results. The read-only Project WOW Environment Brief remains development-gated. Issue #388 still tracks its unresolved VoiceOver Headings-rotor and bidirectional Full Keyboard Access behavior, so the production first-run route remains on the existing onboarding flow.
...
- published opt-in prerelease: **0.19.0-rc.6**, represented by a published GitHub prerelease with signed/notarized GUI artifacts and direct CLI artifacts; GitHub `releases/latest` remains stable-only on `v0.18.2`
- prepared source candidate: **0.19.0-rc.7**; candidate readiness completed on 2026-08-15 from merged `dev` revision `16127dfff2050a2ba2d8b0b3502e110ab4460080`, including focused connectivity QA, the full quality gate, 153 arm64 macOS tests, release/appcast contracts, and non-mutating release checks. RC7 remains untagged and unpublished, and all protected-integration and clean-`main` release gates remain pending
...
- public default-channel GUI appcast and stable CLI metadata remain on signed `v0.18.2` artifacts, while the `beta` appcast item and `latest-rc.json` point to `v0.19.0-rc.6`.
