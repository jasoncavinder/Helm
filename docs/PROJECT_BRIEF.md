...
Third-party Sparkle apps may appear in reviewed plans, but remain interactive vendor-managed steps until Helm has a trustworthy per-app completion contract. They must not be counted as automatic bulk work. Open App is the reliable baseline action; Helm must not promise direct installation from its sandbox unless the target is handled through a separately reviewed, signed/notarized external-updater boundary and completion is verified by observing the installed version. Automatic Plan completion must hand off any remaining interactive steps without claiming that they completed. True direct third-party Sparkle installation is planned for `0.20.x` and must preserve Helm's sandbox rather than broadening its privileges.
...
See docs/legal/LICENSING_STRATEGY.md for planned post-1.0 licensing and distribution model.
