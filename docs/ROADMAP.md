...
- PR #534 source `2c8726a0` implements the Environment Brief responsive-header accessibility correction and passes responsive/all-locale regressions plus the 338-test arm64 UI gate. Its first 2026-08-25 owner artifact lacks a retained exact source revision, tracked-clean tree, or source-blob equivalence mapping, so those observations remain bounded unattributed evidence. A later tracked-clean arm64 Debug artifact from exact source `4b465342` and tree `7e8a425b` passes the wide and Debug-forced-vertical Headings-menu, focus-geometry, `Software sources, 50%` announcement-order, bidirectional Full Keyboard Access, readiness-order/case, and unclipped-layout matrix, and its full arm64 UI gate passes 339 tests with 0 failures, closing Issue #388's acceptance criteria without approving the production route or constituting participant validation.
