...
- Implementation checkpoint: the approved fixed-footprint composition, shared semantic Course projection, environment-domain route, single context row, three fast commands, native utility menu, and Dashboard-owned detail/search/confirmation surfaces are implemented. A debug-only six-state seam renders the production popover without submitting refresh or Helm update checks; the owner state/accessibility matrix is complete; reusable per-session/aggregate records and the validated `v0.20-whole-workflow-v1` synthetic corpus are defined. Production destination projection, the moderated participant checkpoint, and the remaining v0.20 workflow redesign continue before release sign-off.
