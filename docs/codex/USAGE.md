...
```bash
scripts/codex/session.sh "Implement <change> in core/rust; run targeted tests; update source-of-truth docs if behavior changed."
```
...
```text
This flow repeats. Convert it into a skill under skills/<name>/ with SKILL.md and scripts when helpful. Keep release-like paths dry-run/checklist-first.
```
...
Codex should use `skill-generator` to scaffold a new skill under `skills/<skill-name>/` with a generated `SKILL.md` and optional `scripts/`/`resources/`.
...
```text
Use skill: skill-generator to create a reusable skill for this workflow, then update docs/codex/USAGE.md with how to invoke it.
```

Script usage:

```bash
skills/skill-generator/scripts/create_skill.sh --init-spec /tmp/new-skill-spec.md
skills/skill-generator/scripts/create_skill.sh --spec /tmp/new-skill-spec.md --confirm-name
```
...
```bash
scripts/codex/skill-mine.sh
```

Then draft an approved candidate:

```bash
scripts/codex/draft-skill-from-candidate.sh <candidate-name>
```

What this does:

1. extracts the candidate `WORKFLOW SPEC` from `docs/codex/SKILL_CANDIDATES.md`,
2. calls the existing `skill-generator` flow to draft `skills/<candidate-name>/`,
