...
- Procedural workflows are moved to `skills/` or `docs/codex/USAGE.md`.
...
- Script: `scripts/codex/notify-turn-complete.sh`
...
4. update `docs/codex/USAGE.md` with invocation guidance.
...
If additional apps are installed later, re-run `/apps` and extend the task-to-app matrix in `docs/codex/USAGE.md`.
...
Safe optional automation can regenerate `docs/codex/SKILL_CANDIDATES.md` weekly without creating skills.
  ...
  skill-candidates:
    ...
    steps:
      ...
      - name: Generate candidate report
        run: scripts/codex/skill-mine.sh
      - name: Upload candidate report artifact
        uses: actions/upload-artifact@v4
        with:
          name: skill-candidates
          path: docs/codex/SKILL_CANDIDATES.md
```
