...
|---|---|---|---|---|---|---|---|---|
| Status item icon and badge | `AppDelegate.swift`, `AppDelegate+StatusBadge.swift`, `Assets.xcassets/MenuBarIcon.imageset` | Ambient health and entry point | `NSStatusItem` template image, menu title/accessibility description | Simplify | `NSStatusItem` is correct; manual micro-badges need fewer semantics and stronger testing. | Preserve textual tooltip/label; distinguish state without color; test VoiceOver status menu. | v0.20 | Medium: status priority changes can hide urgent state. |
| Right-click status menu | `AppDelegate.configureStatusMenu()` | Fast global commands | `NSMenu` backed by shared command routing | Retain/simplify | Native menu behavior is correct; contents and empty key equivalents are not. | Keyboard equivalents and VoiceOver item state come from shared commands. | v0.19 | Low: route existing selectors through command model. |
