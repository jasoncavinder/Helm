...
| Status banner | `PopoverStatusBanner` | Highest-priority state and action | Compact status callout / native button | Simplify/retain exception | Popover needs one domain-specific triage summary using the shared semantic status system. | Heading + consequence + one action; announce only on meaningful change. | v0.20 | Low. |
