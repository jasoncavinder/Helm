...
- The automated 20,000-row performance gate is complete. A Debug-only, release-optimized arm64 harness measures 30 cold complete projections, 30 warm broad searches, and 120 real native-table viewport/layout samples while bypassing service, updater, database, and network startup. Current-host evidence records cold complete p95 `80.650 ms`, warm median/p95 `10.406/10.589 ms`, 119 of 120 scroll-critical samples within `16.7 ms`, a `32.377 ms` worst sample, and 12 retained row views. The later visible-window Instruments/compositor pass remains a v0.22 requirement.
