...
- Production remote-search submissions are now bound to normalized query generations. Superseding input cancels only task IDs owned by the prior interactive session, late stale callbacks cancel on arrival, description lookups remain independent, polling only retires known terminal IDs, and the existing 500 ms graceful near-completion window remains intact. Focused arm64 macOS tests cover replacement, callback, failure, terminal, and reset races. The 20,000-row performance budget remains later Slice 20.3 work.
