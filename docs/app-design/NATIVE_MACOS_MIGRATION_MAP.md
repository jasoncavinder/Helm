...
- The projector and validator fail closed on scenario-order, any canonical manager record, provenance/policy, or decision drift. A selected missing or invalid dataset presents explicit unavailable Environment and inspector states without exposing production controls, while the whole-workflow selector continues to block service, database, network, updater, detection, and mutation startup. Arm64 app-target tests and locale integrity/length/parity gates pass. The `d60aad9b` behavior pass is historical superseded evidence only. The `d2351667` matrix remains attributable for unaffected grouping/interaction, source-state-first health behavior, exact provenance/policy truth, read-only and reversible decision scope, selected-missing-data fail-closed behavior, Full Keyboard Access, locale refresh, localization, and layout observations. Independent review proved that its no-Hungarian-English-fallback and complete-VoiceOver-semantics claims were inaccurate; current-source owner checks remain open for inactive-badge minimum/expanded Light/Dark/+40% layout, selected-row/inactive-instance VoiceOver semantics, and corrected Hungarian health presentation.

- The `890cf4fe` follow-up preserves the Task 5 implementation contract while correcting the semantic Hungarian Refreshing value and live-refreshing hidden AppKit accessibility window titles. Automated evidence now spans 321 app-target tests; no manual owner validation is attributed to that commit. The earlier 13:56 HST localization, window, inactive-badge layout/+40%, and selected/inactive VoiceOver observations are retained only as bounded unattributed pre-commit evidence and do not close the current-source checks listed above.
