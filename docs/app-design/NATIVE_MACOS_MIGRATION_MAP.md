...
- Task 5 projects every canonical corpus manager through the shipping authority-grouped Environment view and selects the exact rustup finding in the contextual inspector. The inspector identifies `<home>/.cargo/bin/rustup` as active, rustup-installer, and manageable; identifies `/usr/bin/rustup` as inactive, system, and policy-blocked; explains that authorization cannot override policy; exposes only expandable read-only details plus a fixture-local reversible Keep Multiple acknowledgment; localizes live without reselection; and announces each disclosure's expanded/collapsed state. Failed, deferred, review, detected, and not-installed manager health remain distinct.
- The projector and validator fail closed on scenario-order, any canonical manager record, provenance/policy, or decision drift. A selected missing or invalid dataset presents explicit unavailable Environment and inspector states without exposing production controls, while the whole-workflow selector continues to block service, database, network, updater, detection, and mutation startup. Arm64 app-target tests and locale integrity/length/parity gates pass. The owner behavior pass on source commit `d60aad9b` remains bounded pre-remediation evidence. The complete attributable matrix on remediated PR #528 head `d2351667` covers grouping and full-row selection, source-state-first health, exact provenance/policy truth, read-only and reversible decision scope, selected-missing-data fail-closed presentation, minimum/expanded Light/Dark, Full Keyboard Access, VoiceOver, live German/Hungarian/Japanese localization, and approximately +40% expansion.
