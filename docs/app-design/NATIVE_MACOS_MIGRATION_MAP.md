...
- Owner-assisted Full Keyboard Access and VoiceOver execution found the header rotor/traversal defect tracked in Issue #388. Setup-session/receipt mapping and contextual manager inspection moved into the later Task 7 work; the attributable 2026-08-26 matrix closes #388. The separately reviewed default-route replacement remains open.
...
- PR #534 source `2c8726a0` replaces the duplicated Environment Brief header branches with one stable responsive subtree, exposes the rendered state title as a native heading, keeps the Course Indicator separately available without an interaction stop, and localizes title-before-count readiness announcements. Automated responsive/all-locale coverage and the 338-test arm64 gate pass. The 2026-08-25 owner artifact has no retained exact source revision, tracked-clean tree, or source-blob equivalence mapping, so its observations remain bounded unattributed historical evidence.
- PR #535 code source `410abbf9` combines the Course Indicator title and zero-fraction-digit percentage into one selected-locale accessibility label without adding an interaction stop; the all-locale composed-label regression and 339-test arm64 UI gate pass. Its retained owner artifact contains changed objects compiled before `4b465342` was committed, so those observations remain bounded pre-commit, plausibly source-equivalent historical evidence.
- On 2026-08-26, one isolated arm64 Debug artifact built from tracked-clean merged `dev` source `86f4ef23db5081cfce6c595f7d088e1f323a1023` and tree `0597de3a7817347eb5ce3fd7b6dabc570c88141c` passed 339 Xcode tests with 0 failures and the complete owner ordinary-wide/Debug-forced-vertical matrix. Its executable SHA-256 was `3dd0cf112f0c2703974d2d3dded7bf44e08c5b491e5cd6634f2b00e14696a83f`; `Helm.debug.dylib` was `7b210e581e9491475c04d723c4acd9b7411be13be26bfa0c92a2b992c496c6d8`. VoiceOver exposed the title in Headings with matching rendered focus geometry and the separate `Software sources, 50%` element; Full Keyboard Access traversed symmetrically in both directions; and both layouts were unclipped. This closes Issue #388 and the combined Task 7 accessibility item. It does not constitute participant validation or approve the separate production first-run route switch.
...
- Task 7 checkpoint: the final whole-workflow scenario now projects the canonical Environment Brief, setup session, reviewed safe recommendation, verified progress, Action Receipt, and strict aggregate summary through the shipping first-run host behind paired Debug-only selectors. Exact record order and full canonical snapshot identity are validated; selected missing or invalid paired-preview data presents unavailable truth; flow transitions and copied-summary fields are fixture-local; and completion cannot persist onboarding or start live service, database, network, updater, detection, authorization, or mutation work. Focused arm64 contract and locale-parity evidence plus the bounded owner canonical-flow, summary-redaction, and non-persistent relaunch checks pass on app source `bfbea8c6`. The recorded Task 7 exact-source owner checks on `e87d30d3` include scoped VoiceOver stage focus/order/roles and detailed recommendation/progress/receipt/action-state semantics; the later tracked-clean `86f4ef23` owner matrix closes the Environment Brief heading-rotor and bidirectional Full Keyboard Access gate in Issue #388. Whole-workflow cross-cutting accessibility, the Ventura lane, targeted Sparkle handoff, participant validation, and the separate production first-run route decision remain open.
...
- The canonical `v0.20-whole-workflow-v1` corpus and typed semantic validator cover all seven moderated tasks without host scanning or mutation. The Debug-only absolute-path selector now projects Task 1 through shared status-item/popover/Dashboard/sidebar ambient truth, exact affected-manager Environment routes, and the failed-verification Activity inspector/focus handoff; Task 2 through Plan; Task 3 through Library/global search; Task 4 through Activity/inspector; and Task 5 through Environment/manager provenance. Task 1 provider/projection/policy/request-state unit evidence, the full app-target build, and attributable current-host owner validation of production overview/navigation wiring plus SwiftUI keyboard/VoiceOver focus transfer are complete on `3de57a81`; the retained earlier build remains directional evidence only. Task 5 automated contract evidence is complete. The `d2351667` owner matrix remains attributable only for unaffected behavior, Full Keyboard Access, localization, and layout observations: independent review proved its no-Hungarian-English-fallback and complete-VoiceOver-semantics claims inaccurate. Current source adds explicit Active/Inactive instance badges and accessibility values, selected-row `.isSelected` semantics, corrected Hungarian health labels, and targeted locale regressions; an attributable owner matrix on exact app source `9e3fa24a` closes inactive-badge minimum/expanded Light/Dark/+40% layout, selected/inactive VoiceOver semantics, live Hungarian presentation and accessibility-title refresh, locale-specific Settings labels, and independent Settings behavior. The bounded `d60aad9b` pass is historical superseded evidence only. Task 6 now projects the canonical launch-at-login preference through shipping General Settings with fixture-local state and no `SMAppService` access, while the existing Activity `7001` inspector owns the strictly redacted diagnostics action; owner validation is complete on exact app source `43f15385`. Task 7 now projects the final workflow slice behind its paired Debug-only selectors, and its recorded exact-source owner checks on `e87d30d3` include detailed recommendation/progress/receipt/action-state semantics while the later tracked-clean `86f4ef23` matrix closes the Issue #388 Environment Brief accessibility gate.
- Task 6 independent review tightened that boundary across the whole Settings surface: current source prevents privileged-helper `SMAppService` construction/status/registration work and CLI-shim inspection/install/removal for either research fixture, presents explicit localized unavailable truth for invalid or safety-blocked state, and passes a 328-test arm64 gate. The `43f15385` owner matrix remains attributable to its canonical exact-source launch-to-Activity flow; a separate isolated owner pass on exact remediated source `c80d362c` closes the unavailable/helper/CLI presentation, minimum/expanded Light/Dark, VoiceOver, and English/Hungarian/Japanese checks before the research checkpoint.

- PR #528 Task 5 source through `890cf4fe` additionally corrects Hungarian in-progress Refreshing to `Frissítés folyamatban` and reapplies the hidden AppKit Dashboard and Settings accessibility titles on locale change; targeted regressions bring the arm64 app-target gate to 321 tests. The 13:56 HST owner artifact remains bounded unattributed evidence because it predates `061e9fa2` and has no retained source mapping. A fresh isolated arm64 Debug app built from exact app source `9e3fa24a3ba6f9945939b8781661beea4c3ae77f` and tracked-clean tree `fdfc45a8da5d67e4f6262752b6bb47b91fa7f69f` closes inactive-badge minimum/expanded Light/Dark/+40% layout, selected/inactive VoiceOver semantics, complete Hungarian Task 5 presentation including in-progress Refreshing, live Dashboard/Settings accessibility-title updates, locale-specific Settings labels, and independent Settings behavior.
...
- Production remote-search submissions are now bound to normalized query generations and carry explicit interactive or description request purpose across XPC/FFI into Rust in-flight deduplication, preventing cross-purpose task-ID reuse. Superseding input cancels only task IDs owned by the prior interactive session, including late stale callbacks, through a dedicated local-runtime path whose 500 ms graceful near-completion window treats verified already-terminal Search races as benign while surfacing real failures. Snapshot reconciliation retires explicitly owned task IDs on terminal status or after two consecutive authoritative snapshot absences without adopting unrelated work. Focused macOS and Rust tests cover request-purpose isolation, replacement, callback, cancellation failure, terminal and authoritative-absence retirement, and reset races.
- The automated 20,000-row performance gate is complete. A Debug-only, release-optimized arm64 harness measures 30 cold indexed derivations through the shared production row-projection cache, 30 warm cache hits, and 120 real native-table viewport/layout samples while bypassing service, updater, database, and network startup. It uses unique artifacts and requires three host-readiness samples before execution. Current-host evidence records cold indexed/end-to-end p95 `135.458/195.415 ms`, warm indexed median/p95 `22.365/24.332 ms`, warm production end-to-end p95 `30.273 ms`, 117 of 120 scroll-critical samples within `16.7 ms`, a `43.616 ms` worst sample, and 13 retained row views. The later visible-window Instruments/compositor pass remains a v0.22 requirement.
- Task 4's exact applied-but-unverified npm record and unstarted App Store record now project through Activity and its contextual inspector. Apply, verification, before/after, unchanged state, and rollback limits remain separate localized facts. Retry Verification and Keep Change open bounded read-only reviews, Restore is visibly unavailable without a verified downgrade path, and Copy Diagnostics emits only the synthetic strict-redaction summary. The projector rejects scenario, record, action-order, state, availability, and safety drift before presentation; current-host owner visual and accessibility validation remains open.
...
- Task 5 projects every canonical corpus manager through the shipping authority-grouped Environment view and selects the exact rustup finding in the contextual inspector. The selected Environment row carries `.isSelected` accessibility semantics. The inspector identifies `<home>/.cargo/bin/rustup` as active, rustup-installer, and manageable; identifies `/usr/bin/rustup` as inactive, system, and policy-blocked; renders an explicit Active or Inactive badge and accessibility value for every instance; explains that authorization cannot override policy; exposes only expandable read-only details plus a fixture-local reversible Keep Multiple acknowledgment; localizes live without reselection; and announces each disclosure's expanded/collapsed state. Hungarian health labels for Healthy, Error, and Not Installed are localized and protected by a targeted regression. Failed, deferred, review, detected, and not-installed manager health remain distinct.
- The projector and validator fail closed on scenario-order, any canonical manager record, provenance/policy, or decision drift. A selected missing or invalid dataset presents explicit unavailable Environment and inspector states without exposing production controls, while the whole-workflow selector continues to block service, database, network, updater, detection, and mutation startup. Arm64 app-target tests and locale integrity/length/parity gates pass. The `d60aad9b` behavior pass is historical superseded evidence only. The `d2351667` matrix remains attributable for unaffected grouping/interaction, source-state-first health behavior, exact provenance/policy truth, read-only and reversible decision scope, selected-missing-data fail-closed behavior, Full Keyboard Access, locale refresh, localization, and layout observations. Independent review proved that its no-Hungarian-English-fallback and complete-VoiceOver-semantics claims were inaccurate; a fresh isolated arm64 Debug app built from exact app source `9e3fa24a` closes inactive-badge minimum/expanded Light/Dark/+40% layout, selected-row/inactive-instance VoiceOver semantics, corrected Hungarian presentation, live accessibility-title refresh, locale-specific Settings labels, and independent Settings behavior.

- The `890cf4fe` follow-up preserves the Task 5 implementation contract while correcting the semantic Hungarian Refreshing value and live-refreshing hidden AppKit accessibility window titles. Automated evidence spans 321 app-target tests. The earlier 13:56 HST localization, window, inactive-badge layout/+40%, and selected/inactive VoiceOver observations remain bounded unattributed pre-commit evidence. The later exact-source `9e3fa24a` owner matrix closes the current-source checks listed above with retained app, dylib, fixture, source-tree, and +40% resource hashes.
