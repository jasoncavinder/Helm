...
- The projector and validator fail closed on scenario-order, any canonical manager record, provenance/policy, or decision drift. A selected missing or invalid dataset presents explicit unavailable Environment and inspector states without exposing production controls, while the whole-workflow selector continues to block service, database, network, updater, detection, and mutation startup. Arm64 app-target tests and locale integrity/length/parity gates pass. The owner behavior pass on source commit `d60aad9b` covers the pre-remediation grouping, full-row selection, provenance/policy truth, read-only action scope, and reversible acknowledgment; repeat those behavior-sensitive paths on the remediated head together with minimum/expanded Light/Dark, Full Keyboard Access, VoiceOver, and localization validation.
